PRO hadgem3kpp_fcorr_make_smooth_correction,infile,outfile,in_varname,out_varname,n_days_per_year,mean_length
   ...
   FOR j=0,n_lat-1 DO BEGIN
    ...
    ;     ENDIF
         FOR m=0,n_days_per_year-1 DO $
            sfcorr_out(i,j,m)=MEAN(sfcorr_in(i,j,m:n_days-1:n_days_per_year))
         temp=REFORM([REFORM(sfcorr_out(i,j,*)),REFORM(sfcorr_out(i,j,*))])
         temp=SMOOTH(temp,mean_length)
         sfcorr_out(i,j,*)=[temp(n_days_per_year:n_days_per_year+mean_length/2-1),$
...
varids(4)=NCDF_VARDEF(id,out_varname,[dimids(0),dimids(1),dimids(3)])
