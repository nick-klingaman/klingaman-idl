...
global_depth=OPEN_AND_EXTRACT(ocnT_infile,'z')
...
FOR i=0,global_nlon-1 DO BEGIN
   FOR j=0,global_nlat-1 DO BEGIN
      ...
      IF landfrac(i,j) lt 1 THEN BEGIN
         ...
         IF TOTAL(where(thispt_ukmo_ocnT_mmean(*,0) gt 1e10)) ge 0 THEN BEGIN
            max_depth=MIN(where(thispt_ukmo_ocnT_mmean(*,0) gt 1e10))
         ENDIF ELSE $
...
ocnT_varid=NCDF_VARDEF(id,'salinity',[lon_dimid,lat_dimid,z_dimid,t_dimid])
