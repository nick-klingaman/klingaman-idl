PRO hadgem3kpp_fcorr_calculate_expostfacto_perfectfluxes_10m24h,id,month

; Input parameter "month" is the three-letter code for the month for
; which to compute the flux correction.

; Calculate the monthly mean flux correction at the surface using
; Hilary's formula, with temperatures compared to FOAM

; All input files must already be monthly means.

noflx_coupled_indir='/home/ss901165/kpp_ocean2/flxcorr_hadgem3_n96_10mtop_24hr/'+id
...
foam_infile='/home/ss901165/datasets/NCOF_OCEAN/FOAM_bymonth/FOAM_'+month+'mean_mixedlayer_temperature.clim.n96.nc'
...
global_depth=OPEN_AND_EXTRACT(foam_infile,'z')
...
foam_temps_mmean = REFORM(OPEN_AND_EXTRACT(foam_infile,'sea_water_potential_temperature',$
        ...
        FOR k=0,coupled_nz-1 DO BEGIN
            IF (ABS(fcorr_kppgrid(i,j,k)) gt 40) THEN BEGIN
               ...
               ENDIF ELSE IF (j eq coupled_nlat-1 and i ne 0) THEN BEGIN
                  fcorr_kppgrid(i,j,*) = (fcorr_kppgrid(i+1,j,*)+fcorr_kppgrid(i+1,j-1,*)+fcorr_kppgrid(i-1,j,*)+$
                                          fcorr_kppgrid(i-1,j-1,*)+fcorr_kppgrid(i,j-1,*))/5.
               ENDIF ELSE IF (i eq coupled_nlon-1 and j ne 0) THEN BEGIN
                  fcorr_kppgrid(i,j,*) = (fcorr_kppgrid(i-1,j,*)+fcorr_kppgrid(i-1,j+1,*)+fcorr_kppgrid(i-1,j-1,*)+$
                                          fcorr_kppgrid(i,j+1,*)+fcorr_kppgrid(i,j-1,*))/5.
               ENDIF ELSE $
                  fcorr_kppgrid(i,j,*) = (fcorr_kppgrid(i+1,j,*)+fcorr_kppgrid(i+1,j+1,*)+fcorr_kppgrid(i+1,j-1,*)+$
...
; read in by KPP.
id = NCDF_CREATE('/home/ss901165/datasets/HADGEM3-KPP_ANCIL/heat_corrections/'+id+'_flxcorr_10mtop_24hr.'+month+'_mmean.n96.nc',/CLOBBER)
