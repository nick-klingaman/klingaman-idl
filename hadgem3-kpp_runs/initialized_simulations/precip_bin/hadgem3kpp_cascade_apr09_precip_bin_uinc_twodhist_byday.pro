PRO hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday
...
FOR i=0,n_models-1 DO BEGIN
   ...
   rholvl_pres=REFORM(OPEN_AND_EXTRACT(rholvl_pres_file,'p_rholvl',$
                                       ...
                                       count=[model_nlon,model_nlat,model_nz,model_ntimes_per_day*n_days]))/100.
...
;      binned_model_uinc_conv_wshear=fltarr(n_models,n_bins,model_nz)
   ...
   FOR m=0,n_days-1 DO BEGIN
      ...
      model_uinc_total=REFORM(OPEN_AND_EXTRACT(uinc_total_file,'uinc_total',$
                                               offset=[model_ubox_tx(1),model_ubox_tx(0),0,m*model_ntimes_per_day],$
                                               count=[model_nulon,model_nulat,model_nz,model_ntimes_per_day]))*model_incmult
      ...
      model_precip=REFORM(OPEN_AND_EXTRACT(precip_file,'cvrain',$
                                           offset=[model_precipbox_tx(1),model_precipbox_tx(0),0,m*model_ntimes_per_day],$
                                           count=[model_npreciplon,model_npreciplat,1,model_ntimes_per_day]))*(3.)
      ...
      model_zsing_uinc_conv=REFORM(model_uinc_total(*,*,model_zsing,*))
      ...
      FOR j=0,n_bins-1 DO $
         hist_bins(j+1)=model_nonzero_precip(model_nonzero_precip_sorted(FLOOR(N_ELEMENTS(model_nonzero_precip)*j/FLOAT(n_bins))))
      print,hist_bins
      
      model_precip_indices=VALUE_LOCATE(hist_bins,model_precip)
      ...
      PDF,REFORM(model_zsing_uinc_conv,[model_nulon*model_nulat*model_ntimes_per_day]),xrange=[-0.2,0.2],npdf=41,pdf=temp,/noplot
      ...
      FOR j=0,n_bins-2 DO $
         precip_legend_labels(j)=STRMID(STRTRIM(STRING(hist_bins(j)),1),0,4)+' mm hr!U-1!N <= precip rate < '+$
         STRMID(STRTRIM(STRING(hist_bins(j+1)),1),0,4)+' mm hr!U-1!N'
      ...
      FOR j=0,n_models-1 DO BEGIN
         psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/precip_bin/hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday.'+$
                model_runids(i)+'_total.day'+STRTRIM(STRING(m+1),1)+'.ps'
         PSOPEN,file=psfile,FONT=2,CHARSIZE=90,MARGIN=1400,SPACE2=700,XOFFSET=1000,YOFFSET=1500,TFONT=2,TCHARSIZE=80,/PORTRAIT
         GSET,XMIN=0,XMAX=n_bins,YMIN=1005,YMAX=140
         model_top=model_nz
         ...
         CON,X=indgen(n_bins+1)+0.5,Y=rholvl_pres_mean(0:model_top-1),FIELD=REFORM(binned_model_uinc_total(i,*,0:model_top-1)),/BLOCK,$
             TITLE='Total zonal-wind increment binned by precip - 5S-5N, 70E-90E - day '+STRTRIM(STRING(m+1),1),/NOLINES
         AXES,XVALS=indgen(n_bins+1),XLABELS=[STRMID(STRTRIM(STRING(hist_bins),1),0,5),'>'+STRMID(STRTRIM(STRING(hist_bins),1),0,5)],$
              YVALS=indgen(18)*(-50)+1000,XTITLE='Precipitation rate (mm hr!U-1!N)',YTITLE='Pressure (hPa)',NDECS=3,ORIENTATION=40
         PSCLOSE,/NOVIEW
         
         psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/precip_bin/hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday.'+$
                model_runids(i)+'_conv.day'+STRTRIM(STRING(m+1),1)+'.ps'
         PSOPEN,file=psfile,FONT=2,CHARSIZE=90,MARGIN=1400,SPACE2=700,XOFFSET=1000,YOFFSET=1500,TFONT=2,TCHARSIZE=80,/PORTRAIT
         GSET,XMIN=0,XMAX=n_bins,YMIN=1005,YMAX=140
         model_top=model_nz
         ...
         CON,X=indgen(n_bins+1)+0.5,Y=rholvl_pres_mean(0:model_top-1),FIELD=REFORM(binned_model_uinc_conv(i,*,0:model_top-1)),/BLOCK,$
             TITLE='Zonal-wind increment from convection binned by precip - 5S-5N, 70E-90E - day '+STRTRIM(STRING(m+1),1),/NOLINES
         AXES,XVALS=indgen(n_bins+1),XLABELS=[STRMID(STRTRIM(STRING(hist_bins),1),0,5),'>'+STRMID(STRTRIM(STRING(hist_bins),1),0,5)],$
              YVALS=indgen(18)*(-50)+1000,XTITLE='Precipitation rate (mm hr!U-1!N)',YTITLE='Pressure (hPa)',NDECS=3,ORIENTATION=40
         PSCLOSE,/NOVIEW
   
         psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/precip_bin/hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday.'+$
                model_runids(i)+'_advect.day'+STRTRIM(STRING(m+1),1)+'.ps'
         PSOPEN,file=psfile,FONT=2,CHARSIZE=90,MARGIN=1400,SPACE2=700,XOFFSET=1000,YOFFSET=1500,TFONT=2,TCHARSIZE=80,/PORTRAIT
         GSET,XMIN=0,XMAX=n_bins,YMIN=1005,YMAX=140
         model_top=model_nz
         CS,SCALE=1,NCOLS=N_ELEMENTS(total_levels)+1
         ...
         CON,X=indgen(n_bins+1)+0.5,Y=rholvl_pres_mean(0:model_top-1),FIELD=REFORM(binned_model_uinc_advect(i,*,0:model_top-1)),/BLOCK,$
             TITLE='Zonal-wind increment from advection binned by precip - 5S-5N, 70E-90E - day '+STRTRIM(STRING(m+1),1),/NOLINES
         AXES,XVALS=indgen(n_bins+1),XLABELS=[STRMID(STRTRIM(STRING(hist_bins),1),0,5),'>'+STRMID(STRTRIM(STRING(hist_bins),1),0,5)],$
              YVALS=indgen(18)*(-50)+1000,XTITLE='Precipitation rate (mm hr!U-1!N)',YTITLE='Pressure (hPa)',NDECS=3,ORIENTATION=40
         PSCLOSE,/NOVIEW
         
         psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/precip_bin/hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday.'+$
                model_runids(i)+'_bdylr.day'+STRTRIM(STRING(m+1),1)+'.ps'
         PSOPEN,file=psfile,FONT=2,CHARSIZE=90,MARGIN=1400,SPACE2=700,XOFFSET=1000,YOFFSET=1500,TFONT=2,TCHARSIZE=80,/PORTRAIT
         GSET,XMIN=0,XMAX=n_bins,YMIN=1005,YMAX=800
         ...
         CON,X=indgen(n_bins+1)+0.5,Y=rholvl_pres_mean(0:model_top-1),FIELD=REFORM(binned_model_uinc_bdylr(i,*,0:model_top-1)),/BLOCK,$
             TITLE='Zonal-wind increment from boundary-layer binned by precip - 5S-5N, 70E-90E - day '+STRTRIM(STRING(m+1),1),/NOLINES
         AXES,XVALS=indgen(n_bins+1),XLABELS=[STRMID(STRTRIM(STRING(hist_bins),1),0,5),'>'+STRMID(STRTRIM(STRING(hist_bins),1),0,5)],$
              YVALS=indgen(18)*(-50)+1000,XTITLE='Precipitation rate (mm hr!U-1!N)',YTITLE='Pressure (hPa)',NDECS=3,ORIENTATION=40
         PSCLOSE,/NOVIEW
   
         psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/precip_bin/hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday.'+$
                model_runids(i)+'_conv_singlelvl_'+STRTRIM(STRING(single_level),1)+'hPa.day'+STRTRIM(STRING(m+1),1)+'.ps'
         PSOPEN,file=psfile,FONT=2,CHARSIZE=90,MARGIN=1400,SPACE2=700,XOFFSET=1000,YOFFSET=1500,TFONT=2,TCHARSIZE=80,/PORTRAIT
         ...
         AXES,XVALS=indgen(n_uincconv_bins+1),XLABELS=[STRMID(STRTRIM(STRING(uinc_conv_bins),1),0,6),'>'+STRMID(STRTRIM(STRING(uinc_conv_bins(n_uincconv_bins-1)),1),0,6)],$
              YVALS=indgen(n_bins+2),YLABELS=[STRMID(STRTRIM(STRING(hist_bins),1),0,6),'>'+STRMID(STRTRIM(STRING(hist_bins(n_bins)),1),0,6)],$
...
;psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/precip_bin/hadgem3kpp_cascade_apr09_precip_bin_uinc_twodhist_byday.conv_singlelvl_'+$
;       STRTRIM(STRING(single_level),1)+'hPa_pdf.day'+STRTRIM(STRING(m+1),1)+'.ps'
