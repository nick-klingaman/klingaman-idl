PRO hadgem3kpp_cascade_phase2_verticalxsec_wthreedays
...
first_period=[4,7]
second_period=[20,23]
period_name='day3-minus-day1_second12hr'
...
levels_diff=['-0.51','-0.45','-0.39','-0.33','-0.27','-0.21','-0.15','-0.09','-0.03','0.03','0.09','0.15','0.21','0.27','0.33','0.39','0.45','0.51']
levels_entrain_diff=['-4.4','-3.6','-2.8','-2.0','-1.2','-0.4','0.4','1.2','2.0','2.8','3.6','4.4']
levels_mean_diff=['-2.2','-1.8','-1.4','-1.0','-0.6','-0.2','0.2','0.6','1.0','1.4','1.8','2.2']
;levels_entrain_diff=levels_mean_diff
...
FOR i=0,n_sets-1 DO BEGIN
   ...
   FOR k=0,n_cases-1 DO BEGIN
      infile=um3+'/'+runids(k)+'.new_stash/'+runids(k)+'.'+date_ranges(k)+'_3hrmeans.w_after_ts.nc'
      ...
      IF k lt 4 THEN BEGIN
         first_var=REFORM(OPEN_AND_EXTRACT(infile,'dz_dt',offset=[box_tx(1),box_tx(0),0,first_period(0)],$
                                           count=[n_lon,n_lat,n_z,first_period(1)-first_period(0)+1]))*100.
         second_var=REFORM(OPEN_AND_EXTRACT(infile,'dz_dt',offset=[box_tx(1),box_tx(0),0,second_period(0)],$
                                            count=[n_lon,n_lat,n_z,second_period(1)-second_period(0)+1]))*100.
      ENDIF
...
;                                           count=[precip_nlon,precip_nlat,1,first_period(1)-first_period(0)+1]))*86400.
      ...
      psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_wthreedays.'+date_ranges(k)+'_'+set_name+'.diff_'+period_name+'.ps'
      ...
      CON,X=longitude,Y=thlvl_pres_mean,FIELD=var_diff_lonavg,/NOLINELABELS,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
          CB_TITLE='Difference in vertical velocity (cm s!U-1!N)',$
          TITLE='Difference in vertical velocity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
FOR k=0,n_cases-1 DO BEGIN
   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_wthreedays.'+date_ranges(k)+'_diff_1.5x-1.0x.diff_'+period_name+'.ps'
   PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
   ...
   CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(all_var_diff_lonavg(1,k,*,*))-REFORM(all_var_diff_lonavg(0,k,*,*)),$
       ...
       CB_TITLE='Difference in vertical velocity (cm s!U-1!N)',$
       TITLE='Difference of differences in vertical velocity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
FOR k=0,n_sets-1 DO BEGIN
   FOR m=0,n_lon-1 DO BEGIN
      FOR n=0,n_z-1 DO $
         set_mean(k,m,n)=MEAN(all_var_diff_lonavg(k,0:4,m,n))
      precip_set_mean(k,m)=MEAN(all_precip_lonavg(k,*,m))
   ENDFOR

   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_wthreedays.'+set_names(k)+'_mean.diff_'+period_name+'.ps'
   PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
   CS,SCALE=1,NCOLS=N_ELEMENTS(levels_entrain_diff)+1,/REV
   LEVS,MANUAL=levels_entrain_diff
   GSET,XMIN=box(1),XMAX=box(3),YMIN=1015.,YMAX=100.
   CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(set_mean(k,*,*)),$
       /NOLINELABELS,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
       CB_TITLE='Difference in vertical velocity (cm s!U-1!N)',$
       TITLE='Mean of differences in vertical velocity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_wthreedays.diff_1.5xmean-minus-1.0xmean.diff_'+period_name+'.ps'
PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
...
CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(set_mean(1,*,*))-REFORM(set_mean(0,*,*)),$
    /NOLINELABELS,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
    CB_TITLE='Difference in vertical velocity (cm s!U-1!N)',$
    TITLE='Difference of mean differences in vertical velocity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
