PRO hadgem3kpp_cascade_phase2_verticalxsec_qthreedays
...
first_period=[8,11]
second_period=[16,19]
period_name='day3-minus-day2_first12hr'
...
levels_entrain_diff=['-0.75','-0.65','-0.55','-0.45','-0.35','-0.25','-0.15','-0.05','0.05','0.15','0.25','0.35','0.45','0.55','0.65','0.75']
levels_mean_diff=['-0.375','-0.325','-0.275','-0.225','-0.175','-0.125','-0.075','-0.025','0.025','0.075','0.125','0.175','0.225','0.275','0.325','0.375']
...
FOR i=0,n_sets-1 DO BEGIN
   ...
   FOR k=0,n_cases-1 DO BEGIN
      infile=um3+'/'+runids(k)+'.new_stash/'+runids(k)+'.'+date_ranges(k)+'_3hrmeans.q_after_ts.nc'
      ...
      first_var=REFORM(OPEN_AND_EXTRACT(infile,'q',offset=[box_tx(1),box_tx(0),0,first_period(0)],$
                                        count=[n_lon,n_lat,n_z,first_period(1)-first_period(0)+1]))*1000.
      second_var=REFORM(OPEN_AND_EXTRACT(infile,'q',offset=[box_tx(1),box_tx(0),0,second_period(0)],$
                                         count=[n_lon,n_lat,n_z,second_period(1)-second_period(0)+1]))*1000.
...
;                                           count=[precip_nlon,precip_nlat,1,first_period(1)-first_period(0)+1]))*86400.
      ...
      psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_qthreedays.'+date_ranges(k)+'_'+set_name+'.diff_'+period_name+'.ps'
      ...
      CON,X=longitude,Y=thlvl_pres_mean,FIELD=var_diff_lonavg,/NOLINES,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
          CB_TITLE='Difference in specific humidity (g kg!U-1!N)'
          ;TITLE='Difference in specific humidity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
FOR k=0,n_cases-1 DO BEGIN
   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_qthreedays.'+date_ranges(k)+'_diff_1.5x-1.0x.diff_'+period_name+'.ps'
   PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
   ...
   CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(all_var_diff_lonavg(1,k,*,*))-REFORM(all_var_diff_lonavg(0,k,*,*)),$
       /NOLINES,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
       CB_TITLE='Difference in specific humidity (g kg!U-1!N)',$
       TITLE='Difference of differences in specific humidity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
t_result=intarr(n_sets,n_lon,n_z)
precip_t_result=intarr(n_sets,n_lon)
FOR k=0,n_sets-1 DO BEGIN
   FOR m=0,n_lon-1 DO BEGIN
      FOR n=0,n_z-1 DO BEGIN
         set_mean(k,m,n)=MEAN(all_var_diff_lonavg(k,*,m,n))
         t_result(k,m,n)=T_TEST_ONESAMPLE(REFORM(all_var_diff_lonavg(k,*,m,n)),0,1.65)
      ENDFOR
      precip_set_mean(k,m)=MEAN(all_precip_lonavg(k,*,m))
      precip_t_result(k,m)=T_TEST_ONESAMPLE(REFORM(all_precip_lonavg(k,*,m)),0,1.65)
   ENDFOR

   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_qthreedays.'+set_names(k)+'_mean.diff_'+period_name+'.ps'
   PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
   CS,SCALE=1,NCOLS=N_ELEMENTS(levels_entrain_diff)+1,/REV
   LEVS,MANUAL=levels_entrain_diff
   GSET,XMIN=box(1),XMAX=box(3),YMIN=1015.,YMAX=100.
   ...
   FOR m=1,n_lon-1 DO $
      FOR n=0,n_z-1 DO $
         IF thlvl_pres_mean(n) gt 100 THEN $
            IF t_result(k,m,n) eq 1 THEN $
            GPLOT,X=longitude(m),Y=thlvl_pres_mean(n),SYM=3,SIZE=30  
   AXES,XSTEP=10,YVALS=indgen(10)*(-100)+1000.,YMINOR=indgen(17)*(-50)+950.,$
        XMINOR=5,XTITLE='Longitude (degrees east)',YTITLE='Pressure (hPa)',NDECS=1,/NORIGHT
   GSET,XMIN=box(1),XMAX=box(3),YMIN=0,YMAX=20
   GPLOT,X=longitude,Y=REFORM(precip_set_mean(k,*)*(1-0.05*ODD(k))),STYLE=0,COL=FSC_COLOR('purple'),THICK=150
;   FOR m=0,n_lon-1 DO $
;      IF precip_t_result(k,m) eq 1 THEN $
;         GPLOT,X=longitude(m),Y=precip_set_mean(k,m)*(1-0.05*ODD(k)),SYM=3,SIZE=50,COL=FSC_COLOR('purple')
   GPLOT,X=[box(1),box(3)],Y=[0,0],STYLE=1,THICK=200,COL=FSC_COLOR('purple')
   AXES,XSTEP=10,YSTEP=2,YMINOR=1,YTITLE='Mean precipitation (mm day!U-1!N)',/ONLYRIGHT
   PSCLOSE,/NOVIEW
ENDFOR

psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_qthreedays.diff_1.5xmean-minus-1.0xmean.diff_'+period_name+'.ps'
PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
CS,SCALE=1,NCOLS=N_ELEMENTS(levels_mean_diff)+1,/REV
LEVS,MANUAL=levels_mean_diff
GSET,XMIN=box(1),XMAX=box(3),YMIN=1015.,YMAX=100.
t_result=fltarr(n_lon,n_z)
FOR i=0,n_lon-1 DO $
   FOR j=0,n_z-1 DO $
      t_result(i,j)=T_TEST_ONESAMPLE(REFORM(all_var_diff_lonavg(1,*,i,j))*1.15-REFORM(all_var_diff_lonavg(0,*,i,j)),0,1.65)
precip_t_result=fltarr(n_lon)
FOR i=0,n_lon-1 DO $
   precip_t_result(i)=T_TEST_ONESAMPLE(REFORM(all_precip_lonavg(1,*,i))-REFORM(all_precip_lonavg(0,*,i)),0,1.15)
CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(set_mean(1,*,*)*1.15)-REFORM(set_mean(0,*,*)),$
    /NOLINES,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
    CB_TITLE='Difference in specific humidity (g kg!U-1!N)'
    ;TITLE='Difference of mean differences in specific humidity for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
    ;      ' minus hours '+STRTRIM(STRING(first_period(0)*3),1)+'-'+STRTRIM(STRING((first_period(1)+1)*3),1)+' - 1.5x entrain minus control'
FOR i=1,n_lon-1 DO $
   FOR j=0,n_z-1 DO $
      IF thlvl_pres_mean(j) gt 100 THEN $
         IF t_result(i,j) eq 1 THEN $
            GPLOT,X=longitude(i),Y=thlvl_pres_mean(j),SYM=3,SIZE=30
AXES,XSTEP=10,YVALS=indgen(10)*(-100)+1000.,YMINOR=indgen(17)*(-50)+950.,$
     XMINOR=5,XTITLE='Longitude (degrees east)',YTITLE='Pressure (hPa)',NDECS=1,/NORIGHT
GSET,XMIN=box(1),XMAX=box(3),YMIN=-3,YMAX=3
GPLOT,X=longitude,Y=REFORM(precip_set_mean(1,*)*0.95-precip_set_mean(0,*)),STYLE=0,COL=FSC_COLOR('purple'),THICK=150
FOR i=1,n_lon-1 DO $
   IF precip_t_result(i) eq 1 THEN $
      GPLOT,X=longitude(i),Y=REFORM(precip_set_mean(1,i)*0.95-precip_set_mean(0,i)),COL=FSC_COLOR('purple'),SIZE=60,SYM=3
GPLOT,X=[box(1),box(3)],Y=[0,0],STYLE=1,THICK=200,COL=FSC_COLOR('purple')
