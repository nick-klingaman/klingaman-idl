PRO hadgem3kpp_cascade_phase2_verticalxsec_tthreedays
...
FOR i=0,n_sets-1 DO BEGIN
   ...
   FOR k=0,n_cases-1 DO BEGIN
      infile=um3+'/'+runids(k)+'.new_stash/'+runids(k)+'.'+date_ranges(k)+'_3hrmeans.T_after_advect.nc'
      ...
      first_var=REFORM(OPEN_AND_EXTRACT(infile,'field981',offset=[box_tx(1),box_tx(0),0,first_period(0)],$
                                        count=[n_lon,n_lat,n_z,first_period(1)-first_period(0)+1]))
      second_var=REFORM(OPEN_AND_EXTRACT(infile,'field981',offset=[box_tx(1),box_tx(0),0,second_period(0)],$
...
;                                           count=[precip_nlon,precip_nlat,1,first_period(1)-first_period(0)+1]))*86400.
      ...
      psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_tthreedays.'+date_ranges(k)+'_'+set_name+'.diff_'+period_name+'.ps'
      ...
      CON,X=longitude,Y=thlvl_pres_mean,FIELD=var_diff_lonavg,/NOLINELABELS,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
          CB_TITLE='Difference in temperature (K)',$
          TITLE='Difference in temperature for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
FOR k=0,n_cases-1 DO BEGIN
   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_tthreedays.'+date_ranges(k)+'_diff_1.5x-1.0x.diff_'+period_name+'.ps'
   PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
   CS,SCALE=1,NCOLS=N_ELEMENTS(levels_entrain_diff)+1
   ...
   CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(all_var_diff_lonavg(1,k,*,*))-REFORM(all_var_diff_lonavg(0,k,*,*)),$
       ...
       CB_TITLE='Difference in temperature (K)',$
       TITLE='Difference of differences in temperature for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
FOR k=0,n_sets-1 DO BEGIN
   ...
   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_tthreedays.'+set_names(k)+'_mean.diff_'+period_name+'.ps'
   PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
   CS,SCALE=1,NCOLS=N_ELEMENTS(levels_entrain_diff)+1
   LEVS,MANUAL=levels_entrain_diff
   GSET,XMIN=box(1),XMAX=box(3),YMIN=1015.,YMAX=100.
   CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(set_mean(k,*,*)),$
       /NOLINELABELS,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
       CB_TITLE='Difference in temperature (K)',$
       TITLE='Mean of differences in temperature for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/vertical_xsec/hadgem3kpp_cascade_phase2_verticalxsec_tthreedays.diff_1.5xmean-minus-1.0xmean.diff_'+period_name+'.ps'
PSOPEN,file=psfile,TFONT=2,CHARSIZE=100,MARGIN=2200,SPACE3=500,XOFFSET=1000,YOFFSET=200,FONT=2,TCHARSIZE=100,SPACE2=700
CS,SCALE=1,NCOLS=N_ELEMENTS(levels_mean_diff)+1
LEVS,MANUAL=levels_mean_diff
GSET,XMIN=box(1),XMAX=box(3),YMIN=1015.,YMAX=100.
CON,X=longitude,Y=thlvl_pres_mean,FIELD=REFORM(set_mean(1,*,*))-REFORM(set_mean(0,*,*)),$
    /NOLINELABELS,POSITIVE_STYLE=0,NEGATIVE_STYLE=2,$
    CB_TITLE='Difference in temperature (K)',$
    TITLE='Difference of mean differences in temperature for hours '+STRTRIM(STRING(second_period(0)*3),1)+'-'+STRTRIM(STRING((second_period(1)+1)*3),1)+$
