PRO hadgem3kpp_cascade_oct02_rmmphase_plus10
 
; Plot RMM phases for Mar 2005 case
...
obs_ndays=41
obs_rmm1=fltarr(obs_ndays)
obs_rmm2=fltarr(obs_ndays)
IF time_offset+obs_ndays gt 366 THEN BEGIN
   count_yearone=366-time_offset
   count_yeartwo=obs_ndays-count_yearone
   obs_rmm1(0:count_yearone-1)=REFORM(OPEN_AND_EXTRACT(obs_rmm_infile,'rmm1',$
                                                       offset=[year_offset,time_offset],count=[1,count_yearone]))
   obs_rmm2(0:count_yearone-1)=REFORM(OPEN_AND_EXTRACT(obs_rmm_infile,'rmm2',$
                                                       offset=[year_offset,time_offset],count=[1,count_yearone]))
   obs_rmm1(count_yearone:obs_ndays-1)=$
      REFORM(OPEN_AND_EXTRACT(obs_rmm_infile,'rmm1',$
                              offset=[year_offset+1,0],count=[1,count_yeartwo]))
   obs_rmm2(count_yearone:obs_ndays-1)=$
      REFORM(OPEN_AND_EXTRACT(obs_rmm_infile,'rmm2',$
                              offset=[year_offset+1,0],count=[1,count_yeartwo]))
ENDIF ELSE BEGIN
   obs_rmm1(0:obs_ndays-1)=REFORM(OPEN_AND_EXTRACT(obs_rmm_infile,'rmm1',$
                                                   offset=[year_offset,time_offset],count=[1,obs_ndays]))
   obs_rmm2(0:obs_ndays-1)=REFORM(OPEN_AND_EXTRACT(obs_rmm_infile,'rmm2',$
                                                   offset=[year_offset,time_offset],count=[1,obs_ndays]))
ENDELSE
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.obs.ps'
...
GSET,XMIN=-2.0,XMAX=4.25,YMIN=-3.5,YMAX=3.0
GPLOT,X=REPLICATE(0,3),Y=[-3.5,-1],STYLE=0,THICK=80
GPLOT,X=REPLICATE(0,3),Y=[1,3.0],STYLE=0,THICK=80
GPLOT,X=[1,4.25],Y=REPLICATE(0,3),STYLE=0,THICK=80
GPLOT,X=[-1,-2.0],Y=REPLICATE(0,3),STYLE=0,THICK=80
GPLOT,X=[SQRT(2)/2.,1,3.0],Y=[SQRT(2)/2.,1,3.0],STYLE=0,THICK=80
GPLOT,X=[-SQRT(2)/2.,-1,-2.0],Y=[SQRT(2)/2.,1,2.0],STYLE=0,THICK=80
GPLOT,X=[-SQRT(2)/2.,-1,-2.0],Y=[-SQRT(2)/2.,-1,-2.0],STYLE=0,THICK=80
GPLOT,X=[SQRT(2)/2.,1,3.5],Y=[-SQRT(2)/2.,-1,-3.5],STYLE=0,THICK=80

GPLOT,X=0,Y=-4.25,TEXT='Indian Ocean',ALIGN=0.5,CHARSIZE=100
GPLOT,X=4.75,Y=0,TEXT='Maritime Continent',ALIGN=0.5,CHARSIZE=100,ORIENTATION=90
GPLOT,X=0,Y=3.50,TEXT='Western Pacific',ALIGN=0.5,CHARSIZE=100
GPLOT,X=-3.75,Y=0,TEXT='Western Hemisphere and Africa',ALIGN=0.5,CHARSIZE=100,ORIENTATION=90
GPLOT,X=0,Y=-4.00,TEXT='RMM1'
GPLOT,X=-3.50,Y=0,TEXT='RMM2',ORIENTATION=90
...
GPLOT,X=-1.25,Y=-2.50,TEXT='Phase 2'
GPLOT,X=1.25,Y=-2.50,TEXT='Phase 3'
GPLOT,X=2.25,Y=-1.00,TEXT='Phase 4'
GPLOT,X=3.25,Y=2.00,TEXT='Phase 5'
GPLOT,X=1.00,Y=2.50,TEXT='Phase 6'
GPLOT,X=-1.00,Y=2.50,TEXT='Phase 7'
GPLOT,X=-1.50,Y=1.00,TEXT='Phase 8'
GPLOT,X=-1.50,Y=-1.00,TEXT='Phase 1'
...
ENDFOR
GPLOT,X=obs_rmm1(0)+0.1,Y=obs_rmm2(0),TEXT='29 Oct',ALIGN=0.0
GPLOT,X=obs_rmm1(model_offset),Y=obs_rmm2(model_offset),SYM=4,SIZE=100,/NOLINES
GPLOT,X=obs_rmm1(obs_ndays-1)-0.05,Y=obs_rmm2(obs_ndays-1)+0.25,TEXT='8 Dec'
GPLOT,X=obs_rmm1(obs_ndays-1),Y=obs_rmm2(obs_ndays-1),SYM=1,SIZE=100,/NOLINES

analysis_rmm_file='/home/ss901165/datasets_mango/UM_ANALYSES/cascade_mjo_wh04/metum_analysis.jul-dec_dmeans.2002.rmm_indices.nc'
analysis_time_offset=130        ; Corresponding to start date of model integrations
analysis_ndays=30
...
analysis_rmm2=REFORM(OPEN_AND_EXTRACT(analysis_rmm_file,'rmm2',$
                                      offset=[0,analysis_time_offset],count=[1,analysis_ndays]))
;analysis_rmm1(0:analysis_ndays-1)=analysis_rmm1(0:analysis_ndays-1)-(analysis_rmm1(0)-obs_rmm1(analysis_obs_offset_time))
;analysis_rmm2(0:analysis_ndays-1)=analysis_rmm2(0:analysis_ndays-1)-(analysis_rmm2(0)-obs_rmm2(analysis_obs_offset_time))

;analysis_rmm1(6)=(analysis_rmm1(5)+analysis_rmm1(7))/2.0
;analysis_rmm1=analysis_rmm1+0.8
;analysis_rmm2(6)=(analysis_rmm2(5)+analysis_rmm2(7))/2.0
...
FOR i=0,n_models-1 DO BEGIN
   CASE i OF 
      0 : BEGIN
         runid='xftga'
         dirname='xftga'
         description='Control simulation (xftga)'
      ...
      1 : BEGIN
         runid='xftgb'
         dirname='xftgb'
         description='1.5*entrainment (xftgb)'
         obs_offset_time=10 ; Relative to start date of observations
         start_date='08nov02'
         model_ndays=30
         color='blue'
      ...
      3 : BEGIN
         runid='xftgd'
         dirname='xftgd'
         description='1.5*entrain and no CMT (xftgd)'
         obs_offset_time=10
         start_date='08nov02'
         model_ndays=30
         color='purple'
   ...
   model_rmm2=REFORM(OPEN_AND_EXTRACT(model_infile,'rmm2',$
                                      offset=[0,0],count=[1,model_ndays]))
   model_rmm1(0:model_ndays-1)=model_rmm1(0:model_ndays-1)-(model_rmm1(0)-obs_rmm1(analysis_obs_offset_time))
   model_rmm2(0:model_ndays-1)=model_rmm2(0:model_ndays-1)-(model_rmm2(0)-obs_rmm2(analysis_obs_offset_time))
   model_rmm1(1:15)=a*model_rmm1(1:15)   
...
;   all_speeds_tendays(i)=MEAN(temp_speed(0:9))
   ...
   IF i eq 0 THEN BEGIN
      GPLOT,X=4.75,Y=-1.75,TEXT='Model',ALIGN=0.0
      GPLOT,X=5.50,Y=-1.75,TEXT='RMSE',ALIGN=0.0
      GPLOT,X=5.50,Y=-1.90,TEXT='(forecast)',ALIGN=0.0
      GPLOT,X=6.25,Y=-1.75,TEXT='RMSE',ALIGN=0.0
      GPLOT,X=6.25,Y=-1.90,TEXT='(days 1-10)',ALIGN=0.0
      GPLOT,X=7.10,Y=-1.75,TEXT='Speed',ALIGN=0.0
      GPLOT,X=7.10,Y=-1.90,TEXT='(forecast)',ALIGN=0.0
      GPLOT,X=7.85,Y=-1.75,TEXT='Speed',ALIGN=0.0
      GPLOT,X=7.85,Y=-1.90,TEXT='(days 1-10)',ALIGN=0.0
      ...
      GPLOT,X=4.75,Y=-2.35,TEXT='Observations',ALIGN=0.0,COL=30
      GPLOT,X=7.10,Y=-2.35,TEXT=STRMID(STRTRIM(STRING(MEAN(obs_speed)),1),0,5),ALIGN=0.0,COL=30
      GPLOT,X=7.85,Y=-2.35,TEXT=STRMID(STRTRIM(STRING(MEAN(obs_speed(0:9))),1),0,5),ALIGN=0.0,COL=30
   ENDIF
   GPLOT,X=4.75,Y=-2.75-0.25*(i),TEXT=runid,ALIGN=0.0,COL=31+i
   GPLOT,X=5.50,Y=-2.75-0.25*(i),TEXT=STRMID(STRTRIM(STRING(rmse_pc1(i)+rmse_pc2(i)),1),0,5),ALIGN=0.0,COL=31+i
   GPLOT,X=6.25,Y=-2.75-0.25*(i),TEXT=STRMID(STRTRIM(STRING(rmse_pc1_tendays(i)+rmse_pc2_tendays(i)),1),0,5),ALIGN=0.0,COL=31+i
   GPLOT,X=7.10,Y=-2.75-0.25*(i),TEXT=STRMID(STRTRIM(STRING(MEAN(all_speeds(i,*))),1),0,5),ALIGN=0.0,COL=31+i
   GPLOT,X=7.85,Y=-2.75-0.25*(i),TEXT=STRMID(STRTRIM(STRING(MEAN(all_speeds(i,0:9))),1),0,5),ALIGN=0.0,COL=31+i
ENDFOR

GLEGEND,COL=REVERSE(indgen(n_models)+31),SYM=REVERSE(all_symbols),LABELS=REVERSE(all_descriptions),LEGXOFFSET=12000,LEGYOFFSET=18000

PSCLOSE

psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.phase_angle_amplitude.ps'
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.phase_angle_amplitude.16days.ps'
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.phase_angle_amplitude_error.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,XOFFSET=500,YOFFSET=500,TFONT=2,TCHARSIZE=100
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.phase_angle_amplitude_error.16days.ps'
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.phase_speed.ps'
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct02_rmmphase_plus10.phase_speed.16days.ps'
