PRO hadgem3kpp_cascade_oct08_rmmphase
 
; Plot RMM phases for Oct 2008 case
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.bw_pub.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,SPACE3=1500,XOFFSET=500,YOFFSET=0,TFONT=2,TCHARSIZE=100,CB_WIDTH=110,XSIZE=14000,YSIZE=14000
GSET,XMIN=-2.0,XMAX=3.0,YMIN=-2.0,YMAX=2.00
GPLOT,X=REPLICATE(0,2),Y=[-2.0,-1],STYLE=0,THICK=60
GPLOT,X=REPLICATE(0,2),Y=[1,2.0],STYLE=0,THICK=60
GPLOT,X=[1,3.0],Y=REPLICATE(0,2),STYLE=0,THICK=60
GPLOT,X=[-1,-2.0],Y=REPLICATE(0,2),STYLE=0,THICK=60
GPLOT,X=[SQRT(2)/2.,1,2.0],Y=[SQRT(2)/2.,1,2.0],STYLE=0,THICK=60
GPLOT,X=[-SQRT(2)/2.,-1,-2.0],Y=[SQRT(2)/2.,1,2.0],STYLE=0,THICK=60
GPLOT,X=[-SQRT(2)/2.,-1,-2.0],Y=[-SQRT(2)/2.,-1,-2.0],STYLE=0,THICK=60
GPLOT,X=[SQRT(2)/2.,1,2.0],Y=[-SQRT(2)/2.,-1,-2.0],STYLE=0,THICK=60

GPLOT,X=0,Y=-2.50,TEXT='Indian Ocean',ALIGN=0.5,CHARSIZE=100
GPLOT,X=3.25,Y=0,TEXT='Maritime Continent',ALIGN=0.5,CHARSIZE=100,ORIENTATION=90
...
GPLOT,X=0,Y=-2.35,TEXT='RMM1'
GPLOT,X=-2.55,Y=0,TEXT='RMM2',ORIENTATION=90
...
GPLOT,X=-0.75,Y=-1.75,TEXT='Phase 2'
GPLOT,X=0.75,Y=-1.75,TEXT='Phase 3'
GPLOT,X=2.25,Y=-1.25,TEXT='Phase 4'
GPLOT,X=2.25,Y=1.25,TEXT='Phase 5'
GPLOT,X=1.00,Y=1.50,TEXT='Phase 6'
GPLOT,X=-1.00,Y=1.50,TEXT='Phase 7'
...
GPLOT,X=obs_rmm1,Y=obs_rmm2,STYLE=0,THICK=150
this_color=FSC_COLOR("black",30)
this_color=FSC_COLOR("green",31)
FOR i=0,obs_ndays-2,2 DO BEGIN
   IF i ne model_offset THEN $
      GPLOT,X=obs_rmm1(i),Y=obs_rmm2(i),/NOLINES,SYM=5,SIZE=80,COL=30
ENDFOR
GPLOT,X=obs_rmm1(0),Y=obs_rmm2(0)+0.1,TEXT='1 Oct',ALIGN=0.0
GPLOT,X=obs_rmm1(model_offset),Y=obs_rmm2(model_offset),SYM=3,SIZE=100,/NOLINES
GPLOT,X=obs_rmm1(obs_ndays-1),Y=obs_rmm2(obs_ndays-1)-0.15,TEXT='10 Nov'
GPLOT,X=obs_rmm1(obs_ndays-1),Y=obs_rmm2(obs_ndays-1),SYM=1,SIZE=100,/NOLINES

analysis_rmm_file='/home/ss901165/datasets_mango/UM_ANALYSES/cascade_mjo_wh04/metum_analysis.jun-nov_dmeans.2008.rmm_indices.nc'
analysis_time_offset=132        ; Corresponding to start date of model integrations
analysis_ndays=30
...
FOR i=0,n_models-1 DO BEGIN
   ...
   model_rmm2=REFORM(OPEN_AND_EXTRACT(model_infile,'rmm2',$
                                      offset=[0,0],count=[1,model_ndays]))
   model_rmm1=SMOOTH(model_rmm1,5)
   model_rmm2=SMOOTH(model_rmm2,5)
;   this_color=FSC_COLOR(color,31+i)
   GPLOT,X=model_rmm1,Y=model_rmm2,STYLE=style,COL=FSC_COLOR(color),THICK=150
   FOR j=1,model_ndays-2,2 DO $
      GPLOT,X=model_rmm1(j),Y=model_rmm2(j),/NOLINES,SYM=symbol,SIZE=100,COL=FSC_COLOR(color),STYLE=style
   GPLOT,X=model_rmm1(0),Y=model_rmm2(0),/NOLINES,SYM=3,SIZE=100,COL=FSC_COLOR(color),STYLE=style
   GPLOT,X=model_rmm1(model_ndays-1),Y=model_rmm2(model_ndays-1),/NOLINES,SYM=1,SIZE=100,COL=FSC_COLOR(color),STYLE=style
...
;      GPLOT,X=3.25,Y=-1.10,TEXT='Observations',ALIGN=0.0,COL=30
;      GPLOT,X=5.60,Y=-1.10,TEXT=STRMID(STRTRIM(STRING(MEAN(obs_speed)),1),0,5),ALIGN=0.0,COL=30
;      GPLOT,X=6.35,Y=-1.10,TEXT=STRMID(STRTRIM(STRING(MEAN(obs_speed(0:9))),1),0,5),ALIGN=0.0,COL=30
;   ENDIF
;   GPLOT,X=3.25,Y=-1.30-0.20*i,TEXT=runid,ALIGN=0.0,COL=31+i
;   GPLOT,X=4.00,Y=-1.30-0.20*i,TEXT=STRMID(STRTRIM(STRING(rmse_pc1(i)+rmse_pc2(i)),1),0,5),ALIGN=0.0,COL=31+i
;   GPLOT,X=4.75,Y=-1.30-0.20*i,TEXT=STRMID(STRTRIM(STRING(rmse_pc1_tendays(i)+rmse_pc2_tendays(i)),1),0,5),ALIGN=0.0,COL=31+i
;   GPLOT,X=5.60,Y=-1.30-0.20*i,TEXT=STRMID(STRTRIM(STRING(MEAN(all_speeds(i,*))),1),0,5),ALIGN=0.0,COL=31+i
;   GPLOT,X=6.35,Y=-1.30-0.20*i,TEXT=STRMID(STRTRIM(STRING(MEAN(all_speeds(i,0:9))),1),0,5),ALIGN=0.0,COL=31+i
ENDFOR
...
GLEGEND,STYLE=[all_styles,0],SYM=[all_symbols,5],LABELS=[all_descriptions,'Observations'],LEGXOFFSET=1000,LEGYOFFSET=19000,$
        COL=[FSC_COLOR(all_colors),FSC_COLOR('black')]

PSCLOSE
STOP

psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.phase_angle_amplitude.ps'
...
ENDFOR
AXES,XSTEP=4,XMINOR=1,XTITLE='Days since 11 October 2008',/NOLEFT,/NORIGHT
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.phase_angle_amplitude.16days.ps'
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.phase_angle_amplitude_error.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,XOFFSET=500,YOFFSET=500,TFONT=2,TCHARSIZE=100
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.phase_angle_amplitude_error.16days.ps'
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.phase_speed.ps'
...
GSET,XMIN=0,XMAX=model_max_ndays,YMIN=3,YMAX=30,/YLOG,TITLE='Phase speed for October 2008 case study (angle travelled around RMM1/RMM2 space each day)'
black=FSC_COLOR("black",3)
...
ENDFOR
AXES,YTITLE='Phase speed (lag-1 difference in phase angle; degrees)',XSTEP=4,XMINOR=1,XTITLE='Days since 11 October 2008',NDECS=2,$
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/rmm_phases/hadgem3kpp_cascade_oct08_rmmphase.phase_speed.16days.ps'
