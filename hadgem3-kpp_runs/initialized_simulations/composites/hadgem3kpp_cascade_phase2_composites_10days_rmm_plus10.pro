PRO hadgem3kpp_cascade_phase2_composites_10days_rmm_plus10

; Make day-by-day composites of precip, U850, U200, OLR from
; the phase 2 +10 day integrations for the control and 1.5x entrainment.
...
control_runs=['xftfa','xftfe','xftfi','xftfm','xftfq','xftfu','xftga','xftge','xftgi','xftgm','xftgq','xftgu']
control_dirs=control_runs
entrain_runs=['xftfb','xftff','xftfj','xftfn','xftfr','xftfv','xftgb','xftgf','xftgj','xftgn','xftgr','xftgv']
entrain_dirs=entrain_runs
nocmt_runs=['xftfc','xftfg','xftfk','xftfo','xftfs','xftfw','xftgc','xftgg','xftgk','xftgo','xftgs','xftgw']
nocmt_dirs=nocmt_runs
combine_runs=['xftfd','xftfh','xftfl','xftfp','xftft','xftfx','xftgd','xftgh','xftgl','xftgp','xftgt','xftgx']
combine_dirs=combine_runs

start_dates=['14nov09','05sep08','17dec07','17sep06','28mar05','13dec03','08nov02','15jun02','06may02','26jan01','23nov00','08oct00']
offset_years=[20,       19,       18,       17,       16,       14,       13,       13,       13,       12,       11,       11]+14
offset_dates=[318,      248,      351,      260,      87,       347,      312,      166,      126,      26,       327,      281]-1
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/composites/hadgem3kpp_cascade_phase2_composites_10days_rmm_plus10.entrain_ctl_obs.color.ps'
...
GPLOT,X=0,Y=-3.05,TEXT='Indian Ocean',ALIGN=0.5,CHARSIZE=100
...
GPLOT,X=-2.65,Y=0,TEXT='Western Hemisphere and Africa',ALIGN=0.5,CHARSIZE=100,ORIENTATION=90
GPLOT,X=0,Y=-2.90,TEXT='RMM1'
GPLOT,X=-2.45,Y=0,TEXT='RMM2',ORIENTATION=90
...
FOR i=0,n_sets-1 DO BEGIN
   GPLOT,X=REFORM(model_composite_rmm1(i,*)),Y=REFORM(model_composite_rmm2(i,*)),STYLE=styles(i),COL=FSC_COLOR(colors(i)),THICK=150;STYLE=styles(i)
   FOR j=0,n_days-1,5 DO $
      GPLOT,X=model_composite_rmm1(i,j),Y=REFORM(model_composite_rmm2(i,j)),SYM=syms(i),/NOLINES,SIZE=80,COL=FSC_COLOR(colors(i))
   GPLOT,X=model_composite_rmm1(i,n_days-1),Y=model_composite_rmm2(i,n_days-1),SYM=syms(i),SIZE=80,COL=FSC_COLOR(colors(i))
ENDFOR
black=FSC_COLOR('black',30+n_sets)
GPLOT,X=obs_composite_rmm1(*),Y=obs_composite_rmm2(*),STYLE=0,THICK=150
FOR j=0,n_days-1,5 DO $
   GPLOT,X=obs_composite_rmm1(j),Y=REFORM(obs_composite_rmm2(j)),STYLE=0,SYM=2,/NOLINES,SIZE=80
GPLOT,X=obs_composite_rmm1(n_days-1),Y=obs_composite_rmm2(n_days-1),SYM=2,SIZE=80

labels=['Observations','Control','1.5x entrainment'];,'NoCMT','1.5*epsilon+NoCMT']
GLEGEND,labels=REVERSE(labels),SYM=REVERSE([2,syms]),LEGXOFFSET=2000,LEGYOFFSET=19500,STYLE=REVERSE([0,styles]),COL=REVERSE([FSC_COLOR('black'),FSC_COLOR(colors)]);,STYLE=REVERSE([0,styles])
