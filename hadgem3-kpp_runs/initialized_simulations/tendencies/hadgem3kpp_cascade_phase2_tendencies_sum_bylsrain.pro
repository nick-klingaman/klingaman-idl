PRO hadgem3kpp_cascade_phase2_tendencies_sum_bylsrain
  
; Plot line graphs of the vertical structure of individual physical tendencies for a given bin
; of large-scale precipitation, then plot the sum of all physical tendencies for each variable.
...
FOR i=0,n_deciles-1 DO $
   decile_labels(i)='Decile '+STRTRIM(STRING(precipitation_deciles(i)+1),1)+' of large-scale precip'
...
FOR j=0,n_variables-1 DO BEGIN
   CASE j OF
      2 : BEGIN
         ...
         xmin=[-0.06,-0.12,-0.06]
         xmax=[0.18,0.06,0.12]
         xstep=[0.02,0.01,0.01]
         
         xmin_accel=[-0.28,-0.12,-0.18]         
         xmax_accel=[0.04,0.18,0.02]
      ...
      3 : BEGIN
         ...
         multiplier=3.
         ymax=[850.,100.,100.,100.]
         ystep=[-10.,-100.,-100.,-100.]
         xmin=[-0.15,-0.40,-0.10,-0.30]
         xmax=[0.36,0.40,0.04,0.40]
      ...
      0 : BEGIN
         ...
         xmin=[-0.06,-0.10,-0.90,-0.12]
         xmax=[0.12,0.80,0.10,0.12]
         xstep=[0.02,0.10,0.10,0.02]
         legpos=[[1,9],$
                 [9,11],$
                 [1,9],$
                 [1,9]]
...
;      STOP
   ...
   FOR k=0,n_tendencies-1 DO BEGIN
      psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/tendencies/hadgem3kpp_cascade_phase2_tendencies_sum_bylsrain.'+$
             varname+'_'+tendencies(k)+'.lsrain_deciles';+STRTRIM(STRING(precipitation_deciles+1),1)+'.hours0-71.allexpts.ps'
      ...
      IF varname eq 'Uinc' THEN BEGIN
         psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/tendencies/hadgem3kpp_cascade_phase2_tendencies_sum_bylsrain.'+$
             varname+'_'+tendencies(k)+'.with_acceleration.lsrain_deciles';+STRTRIM(STRING(precipitation_deciles+1),1)+'.hours0-71.allexpts.ps'
         FOR m=0,n_deciles-1 DO $
            psfile=psfile+STRTRIM(STRING(precipitation_deciles(m)+1),1)         
         psfile=psfile+'.hours0-71.allexpts.ps'
         print,psfile
   ...
   ENDFOR
   psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/tendencies/hadgem3kpp_cascade_phase2_tendencies_sum_bylsrain.'+$
          varname+'_sumphys.lsrain_deciles'
   FOR m=0,n_deciles-1 DO $
         psfile=psfile+STRTRIM(STRING(precipitation_deciles(m)+1),1)
   psfile=psfile+'.hours0-71.allexpts.ps'
   print,psfile
   ...
   IF varname eq 'Uinc' THEN BEGIN
      psfile='/home/ss901165/idl/hadgem3-kpp_runs/initialized_simulations/tendencies/hadgem3kpp_cascade_phase2_tendencies_sum_bylsrain.'+$
             varname+'_sumphys.with_acceleration.lsrain_deciles' ;+STRTRIM(STRING(precipitation_deciles+1),1)+'.hours0-71.allexpts.ps'
      FOR m=0,n_deciles-1 DO $
         psfile=psfile+STRTRIM(STRING(precipitation_deciles(m)+1),1)         
      psfile=psfile+'.hours0-71.allexpts.ps'
      print,psfile
