PRO hadgem3kpp_nao_autocorr_daily_crossseason
...
FOR m=0,nmodels-1 DO BEGIN
   CASE m OF
      ...
      END
      1 : BEGIN
         nao_file='/home/ss901165/um_output6/xjhwb/hadgem3kpp_nrglobal_n216.jan-dec_dmeans_ts.years1-106.nao_index_goml_djf.nc'
         ntime=LONG(106)*LONG(360)
         offset=0
         all_names(m)='GOML1, EOF Dec-Feb'
         all_colors(m)='purple'
         all_symbols(m)=4
         all_styles(m)=2
         days=[330,420]        
      ...
      END
      3: BEGIN
         nao_file='/home/ss901165/datasets/20THC_REANALYSIS/mslp/20thc_reanalysis.jan-dec_dmeans_ts.1871-2010.nao_index_20CR_djf.nc'       
         ntime=LONG(140)*LONG(365)
         offset=0
         all_names(m)='20CR, EOF Dec-Feb'
         all_colors(m)='darkgrey'
         all_symbols(m)=5
         all_styles(m)=2
         days=[334,424]
...
psfile='/home/ss901165/idl/hadgem3-kpp_runs/nao/hadgem3kpp_nao_autocorr_daily_crossseason.metum_goml1_n216_owneof_djf.ps'
...
FOR m=0,nmodels-1 DO BEGIN
   GPLOT,X=findgen(nlags),Y=REFORM(autocorr(m,*)),SYM=all_symbols(m),COL=FSC_COLOR(all_colors(m)),STYLE=all_styles(m),SIZE=80
   FOR i=0,2 DO BEGIN
...
GLEGEND,labels=REVERSE(all_names),COL=REVERSE(FSC_COLOR(all_colors)),LEGXOFFSET=0,LEGYOFFSET=16000,LENGTH=80,$
...
FOR m=0,nmodels-1 DO BEGIN
   GPLOT,X=MAX(lags)+1+m/2.,Y=efold_thirds(m,3),SYM=6,COL=FSC_COLOR(all_colors(m))
   IF m ne 10 THEN BEGIN
      GPLOT,X=[MAX(lags)+1+m/2.,MAX(lags)+1+m/2.],Y=[efold_thirds(m,3),MAX(efold_thirds(m,0:2))],COL=FSC_COLOR(all_colors(m)),STYLE=all_styles(m)
      GPLOT,X=[MAX(lags)+1+m/2.,MAX(lags)+1+m/2.],Y=[efold_thirds(m,3),MIN(efold_thirds(m,0:2))],COL=FSC_COLOR(all_colors(m)),STYLE=all_styles(m)
   ENDIF 
      
   GPLOT,X=MAX(lags)+7+m/2.,Y=plev_thirds(m,3),SYM=4,COL=FSC_COLOR(all_colors(m))
   IF m ne 10 THEN BEGIN
      GPLOT,X=[MAX(lags)+7+m/2.,MAX(lags)+7+m/2.],Y=[plev_thirds(m,3),MAX(plev_thirds(m,0:2))],COL=FSC_COLOR(all_colors(m)),STYLE=all_styles(m)
      GPLOT,X=[MAX(lags)+7+m/2.,MAX(lags)+7+m/2.],Y=[plev_thirds(m,3),MIN(plev_thirds(m,0:2))],COL=FSC_COLOR(all_colors(m)),STYLE=all_styles(m)
   ENDIF
ENDFOR
