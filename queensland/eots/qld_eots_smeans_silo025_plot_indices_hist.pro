PRO qld_eots_smeans_silo025_plot_indices_hist
...
FOR i=0,n_indices-1 DO BEGIN
   CASE i OF
      ...
      0 : BEGIN
         ...
         year_ranges=['1957-2010','1957-2010','1957-2010','1957-2010','1957-2010']
   ...
   ENDFOR
   psfile='/home/ss901165/idl/queensland/eots/indices/qld_eots_smeans_silo025_plot_indices_hist.'+psfilename+'.ps'
   ...
   GSET,XMIN=year_start,XMAX=year_start+n_years,YMIN=FLOOR(MIN(seasonal_timeseries(4,*))*1.1),$
        YMAX=FLOOR(MAX(seasonal_timeseries(4,*))*1.1)+1
;   FOR j=0,n_seasons-1 DO $
;      GPLOT,X=indgen(n_years)+year_start,Y=REFORM(seasonal_timeseries(j,*)),COL=FSC_COLOR(colors(j))
   negative=where(timeseries lt 0)
   positive=where(timeseries gt 0)
   xvals=indgen(n_years)+year_start
   HIST,X=xvals[positive],Y=timeseries[positive],FILLCOL=FSC_COLOR('blue')
   HIST,X=xvals[negative],Y=timeseries[negative],FILLCOL=FSC_COLOR('red')
   timeseries_smooth=SMOOTH(timeseries,11)
   GPLOT,X=xvals(5:n_years-6),Y=timeseries_smooth(5:n_years-6),THICK=150,COL=FSC_COLOR('black')
;   GLEGEND,labels=REVERSE(season_names),COL=REVERSE(FSC_COLOR(colors)),LEGPOS=legpos
   AXES,XSTEP=xstep,XMINOR=xminor,YSTEP=ystep,YMINOR=yminor,XTITLE='Year',YTITLE=vardesc,NDECS=2
