PRO qld_eots_transition_probabilities_terciles_silo025
...
openw,lun,'/home/ss901165/idl/queensland/eots/qld_eots_transition_probabilities_terciles_silo025.smeans_yearone.txt',/get_lun
...
FOR i=0,n_seasons-1 DO BEGIN
   ...
   FOR j=0,n_eots-1 DO BEGIN
;      output_string='       '+STRTRIM(STRING(j+1),1)+'   +tr  ('+STRMID(STRTRIM(STRING(N_ELEMENTS(where(silo_smeans_eots(i,*,j) gt 0))/FLOAT(n_years)),1),0,4)+')    +tr       '
      output_string='       '+STRTRIM(STRING(j+1),1)+'   +tr  ('+STRMID(STRTRIM(STRING(0.33),1),0,4)+')    +tr       '
      FOR k=0,n_seasons-1 DO BEGIN
         FOR m=0,n_eots-1 DO BEGIN
            add=0.00
      ...
      ENDFOR
      printf,lun,output_string
;      output_string='       '+STRTRIM(STRING(j+1),1)+'   -tr  ('+STRMID(STRTRIM(STRING(N_ELEMENTS(where(silo_smeans_eots(i,*,j) lt 0))/FLOAT(n_years)),1),0,4)+')    +tr       '
      output_string='       '+STRTRIM(STRING(j+1),1)+'   -tr  ('+STRMID(STRTRIM(STRING(0.33),1),0,4)+')    +tr       '
      FOR k=0,n_seasons-1 DO BEGIN
         FOR m=0,n_eots-1 DO BEGIN
            add=0.00
