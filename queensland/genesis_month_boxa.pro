...
elnino_average=fltarr(n_months)
lanina_average=fltarr(n_months)
FOR i=0,n_months-1 DO BEGIN
   thismonth_genesis=REFORM(average_genesis_months(*,i))
   elnino_average(i)=MEAN(thismonth_genesis[elnino_years])
   lanina_average(i)=MEAN(thismonth_genesis[lanina_years])
ENDFOR


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
psfile='/home/ss901165/idl/queensland/genesis_month_boxa.ps'
...
CS,SCALE=26
GSET,XMIN=0,XMAX=n_months,YMIN=0,YMAX=20
HIST,X=indgen(n_months)+0.5,Y=average_genesis_months,WIDTH=100,FILLCOL=1
AXES,NDECS=2,XTITLE='Month of TC genesis [model output]',YTITLE='[storms year!U-1!N (10!U6!N km)!U-1!N]',YMINOR=0.25
