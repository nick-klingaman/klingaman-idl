PRO qld_higem_eots_smeans_eafeb
...
;box=[-15,138,-10,154]
n_eots=6
sig_level=0.19 ; for 150 years
higem_offset=0
higem_nyears=149
nino_start=0
year_range_str='h9-w8'
...
FOR p=0,n_seasons-1 DO BEGIN
   ...
   higem_smeans_infile='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.'+smean_period+'_smeans.h9-w8.precip.aus_domain.nc'
   nino_infile='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.'+smean_period+'_smeans.h9-w8.nino_indices.nc'
   
                                ; Read latitude and longitude from file
   higem_latitude=OPEN_AND_EXTRACT(higem_smeans_infile,'latitude')
   higem_longitude=OPEN_AND_EXTRACT(higem_smeans_infile,'longitude')
   ...
   nino4_ts=OPEN_AND_EXTRACT(nino_infile,'nino4',offset=[nino_start],count=[higem_nyears])
   nino3_ts=OPEN_AND_EXTRACT(nino_infile,'nino3',offset=[nino_start],count=[higem_nyears])
   nino34_ts=OPEN_AND_EXTRACT(nino_infile,'nino34',offset=[nino_start],count=[higem_nyears])
   
                                ; Read precipitation data, mask, and create an area-averaged
                                ; timeseries.
   higem_smeans=OPEN_AND_EXTRACT(higem_smeans_infile,'precip',$
                                offset=[higem_box_tx(1),higem_box_tx(0),higem_offset],$
                                count=[higem_nlon,higem_nlat,higem_nyears])*86400.
...
;      temp[where(temp ge 1E10)]=!Values.F_NaN      
   ...
   FOR k=0,n_eots-1 DO BEGIN
      ...
      FOR i=0,higem_nlon-1 DO BEGIN
         ;print,'Seaching longitude '+STRTRIM(STRING(i+1),1)+' of '+STRTRIM(STRING(higem_nlon),1)
         FOR j=0,higem_nlat-1 DO BEGIN
            IF FINITE(higem_smeans(i,j,0)) eq 1 and TOTAL(higem_smeans(i,j,*)) ne 0 THEN BEGIN
               ...
               ;higem_smeans_corr_with_aavg(i,j)=CORRELATE(higem_smeans_thispt_ts,higem_smeans_aavg)
               ;regression=REGRESS(higem_smeans_thispt_ts,higem_smeans_aavg,CORRELATION=correlation,CONST=constant)
               ;temp_explained=fltarr(higem_nlon,higem_nlat);higem_nyears)
               variance_explained=0.
               FOR m=0,higem_nlon-1 DO BEGIN
                  FOR n=0,higem_nlat-1 DO BEGIN
                     IF FINITE(higem_smeans(m,n,0)) eq 1 and TOTAL(higem_smeans(m,n,*)) ne 0 THEN BEGIN
                        temp_ts=REFORM(higem_smeans(m,n,*))
;                        temp_stddev=STDDEV(temp_ts)
                        variance_explained=CORRELATE(temp_ts,higem_smeans_thispt_ts)^2*VARIANCE(temp_ts)+variance_explained
                                ;FOR p=0,higem_nyears-1 DO $
                                ;   temp_explained(m,n,p)=higem_smeans_thispt_ts(p)*temp_eot
...
;               total_explained=0
               ;FOR p=0,higem_nyears-1 DO $
               ;   total_explained=TOTAL(REFORM(temp_explained(*,*,p)^2)*higem_aavg_weight,/NaN)+total_explained
               ;total_explained=total_explained/(higem_smeans_stvar/FLOAT(higem_nyears))
               IF variance_explained gt higem_eot_max_explained THEN BEGIN
                  higem_eot_max_explained=variance_explained
                  higem_eot_pt=[i,j]
                  ;print,'Found new base point: '+STRTRIM(STRING(higem_latitude(higem_eot_pt(1))),1)+' '+$
                  ;      STRTRIM(STRING(higem_longitude(higem_eot_pt(0))),1)
               ENDIF
;               IF higem_smeans_corr_with_aavg(i,j) gt higem_eot_max_corr THEN BEGIN
;                  higem_eot_max_corr=higem_smeans_corr_with_aavg(i,j)
;                  higem_eot_pt=[i,j]
;               ENDIF
            ENDIF ELSE $
               higem_smeans_corr_with_aavg(i,j)=!Values.F_NaN
         ENDFOR
      ENDFOR
...
;      print,VARIANCE(higem_smeans_eots_explained,/NaN)/higem_smeans_stvar
   ...
   FOR k=0,n_eots-1 DO BEGIN
      ...
      psfile='/home/ss901165/idl/queensland/higem/eots/qld_higem_eots_eafeb.'+smean_period+'_smeans.eot'+this_eot_str+'.'+year_range_str+'.ps'
...
;   CS, COLS=[93, 123, 91, 267, 272, 367, 409, 419, 407]
      ...
      CON,FIELD=REFORM(higem_smeans_corr_with_eots(this_eot,*,*)),X=higem_longitude,Y=higem_latitude,$
          TITLE='EOT'+this_eot_str+' for HiGEM control, using seasonal means for '+smean_period+' '+year_range_str+'',/NOLINES,/BLOCK
      FOR m=0,higem_nlon-1 DO BEGIN
         FOR n=0,higem_nlat-1 DO BEGIN
      ...
      psfile='/home/ss901165/idl/queensland/higem/eots/qld_higem_eots_eafeb.'+smean_period+'_smeans.eot'+this_eot_str+'_ts.'+year_range_str+'.ps'
      ...
      GSET,XMIN=0,XMAX=150,YMIN=ymin,YMAX=ymax,$
           TITLE='Timeseries of EOT'+this_eot_str+' for HiGEM control, using seasonal means for '+smean_period+' '+year_range_str+''
      black=FSC_COLOR("black",9)
      HIST,X=indgen(higem_nyears)+higem_offset,Y=this_eot_ts,COL=9,WIDTH=50
      AXES,XSTEP=10,YSTEP=100,XMINOR=2,YMINOR=50,YTITLE='EOT timeseries',XTITLE='Year at beginning of '+smean_period+' period'
      GPLOT,X=indgen(higem_nyears)+higem_offset,Y=REPLICATE(0,higem_nyears),STYLE=1
      red=FSC_COLOR("red",10)
      smoothed=SMOOTH(this_eot_ts,11)
      GPLOT,X=indgen(higem_nyears-10)+higem_offset+5,Y=smoothed(5:higem_nyears-6),STYLE=0,THICK=150,COL=10
      GPLOT,X=higem_offset+5,Y=ymax-(ymax-ymin)*0.03,ALIGN=0,TEXT='Explains '+STRMID(STRTRIM(STRING(higem_smeans_eots_expvar(this_eot)*100.),1),0,5)+$
            '% of domain space-time variance'
      GPLOT,X=higem_offset+5,Y=ymax-(ymax-ymin)*0.06,ALIGN=0,TEXT='Correlation with seasonal-mean ('+smean_period+') NINO4: '+$
            STRMID(STRTRIM(STRING(CORRELATE(this_eot_ts,nino4_ts)),1),0,5)
      GPLOT,X=higem_offset+5,Y=ymax-(ymax-ymin)*0.09,ALIGN=0,TEXT='Correlation with seasonal-mean ('+smean_period+') NINO3.4: '+$
            STRMID(STRTRIM(STRING(CORRELATE(this_eot_ts,nino34_ts)),1),0,5)
      GPLOT,X=higem_offset+5,Y=ymax-(ymax-ymin)*0.12,ALIGN=0,TEXT='Correlation with seasonal-mean ('+smean_period+') NINO3: '+$
            STRMID(STRTRIM(STRING(CORRELATE(this_eot_ts,nino3_ts)),1),0,5)
      
      PSCLOSE,/NOVIEW
      
      psfile='/home/ss901165/idl/queensland/higem/eots/qld_higem_eots_eafeb.'+smean_period+'_smeans.eot'+this_eot_str+'_nino4.'+year_range_str+'.ps'
      ...
      GSET,XMIN=0,XMAX=150,YMIN=-1,YMAX=1,$
        TITLE='NINO4 correlations with EOT'+this_eot_str+' for HiGEM control, using ann-means for '+smean_period+' '+year_range_str+''
      GPLOT,X=indgen(higem_nyears)+higem_offset,Y=REPLICATE(0,higem_nyears),STYLE=1
      GPLOT,X=higem_offset+5,Y=0.9,ALIGN=0,TEXT='Explains '+STRMID(STRTRIM(STRING(higem_smeans_eots_expvar(this_eot)*100.),1),0,5)+$
            '% of domain space-time variance'
      GPLOT,X=higem_offset+5,Y=0.85,ALIGN=0,TEXT='Correlation with seasonal-mean ('+smean_period+') NINO4: '+$
            STRMID(STRTRIM(STRING(CORRELATE(this_eot_ts,nino4_ts)),1),0,5)
      GPLOT,X=higem_offset+5,Y=0.8,ALIGN=0,TEXT='Correlation with seasonal-mean ('+smean_period+') NINO3.4: '+$
            STRMID(STRTRIM(STRING(CORRELATE(this_eot_ts,nino34_ts)),1),0,5)
      GPLOT,X=higem_offset+5,Y=0.75,ALIGN=0,TEXT='Correlation with seasonal-mean ('+smean_period+') NINO3: '+$
            STRMID(STRTRIM(STRING(CORRELATE(this_eot_ts,nino3_ts)),1),0,5)
      ...
      FOR i=0,n_windows-1 DO BEGIN
         GPLOT,X=indgen(higem_nyears)+higem_offset,Y=REFORM(this_eot_corr_nino4_windows(i,*)),COL=2+i,THICK=150
         GPLOT,X=indgen(higem_nyears)+higem_offset,Y=REFORM(this_eot_corr_nino4_windows_signif(i,*)),COL=2+i,/NOLINES,SYM=2,SIZE=70
      ENDFOR
   ...
   output_file='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.'+smean_period+'_smeans.'+year_range_str+'.eots.nc'
   ...
   NCDF_VARPUT,id,varids(0),indgen(higem_nyears)+higem_offset
