PRO qld_higem_dblco2_precip_percentiles_range_combined
...
mylevs_ratio=['0.66','0.70','0.74','0.78','0.82','0.86','0.90','0.94','0.98','1.02','1.06','1.11','1.16','1.22','1.28','1.35','1.43','1.52']
...
FOR s=0,n_seasons-1 DO BEGIN
   CASE s OF 
      0 : BEGIN
         ...
         stbl_infile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eafee_eadwu.dec-feb_dmeans.m9-s0.precip.global_domain.nc'
         season_name='dec-feb'
         n_days=90
      END
      1 : BEGIN
         ctl_infile='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.mar-may_dmeans.h9-w8.precip_percentiles.aus_domain.nc'
         stbl_infile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eafee_eadwu.mar-may_dmeans.m9-s0.precip.global_domain.nc'
         season_name='mar-may'
         n_days=90
      END
      4 : BEGIN
         ctl_infile='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.jun-aug_dmeans.h9-w8.precip_percentiles.aus_domain.nc'
         stbl_infile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eafee_eadwu.jun-aug_dmeans.m9-s0.precip.global_domain.nc'
         season_name='jun-aug'
         n_days=90
      END
      3 : BEGIN
         ctl_infile='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.sep-nov_dmeans.h9-w8.precip_percentiles.aus_domain.nc'
         stbl_infile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eafee_eadwu.sep-nov_dmeans.m9-s0.precip.global_domain.nc'
         season_name='sep-nov'
         n_days=90
      END
      2 : BEGIN
         ctl_infile='/home/ss901165/higem_qccce/es_control_eafeb/higem_eafeb.may-apr_dmeans.h9-w8.precip_percentiles.aus_domain.nc'
         stbl_infile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eafee_eadwu.may-apr_dmeans.m9-s0.precip.global_domain.nc'
         season_name='may-apr'
         n_days=360
      END
   ENDCASE
   ...
   FOR i=0,n_ranges-1 DO BEGIN
      ...
      psfile='/home/ss901165/idl/queensland/higem/double_co2/precip_percentiles/qld_higem_dblco2_precip_percentiles_range_combined.higem_ctl.'+season_name+'_range'+STRTRIM(STRING(ranges(i)),1)+'.ps'
      ...
      GSET,YMIN=0,YMAX=(ranges(i)*2+1)^2*(90.*90.)/(100.*100.),XMIN=0,XMAX=n_percentiles
      CON,Y=(indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100.),X=indgen(n_percentiles)+0.5,FIELD=REFORM(ctl_range_count(i,*,0:(ranges(i)*2+1)^2-1)),/BLOCK,$
      ...
      AXES,YVALS=(indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100),YLABELS=STRMID(STRTRIM(STRING((indgen((ranges(i)*2+1)^2)+1)*(90.*90.)/(100.*100)),1),0,4),$
           XVALS=indgen(n_percentiles)+0.5,XLABELS=STRMID(STRTRIM(STRING(percentiles),1),0,4),ORIENTATION=45,$
           YTITLE='Estimated area with precipitation >= percentile (10!U4!N km!U2!N)',$
           XTITLE='Percentile of daily rainfall for '+season_name+' (days >= 1 mm only) from control simulation',NDECS=1
      PSCLOSE,/NOVIEW
      
      psfile='/home/ss901165/idl/queensland/higem/double_co2/precip_percentiles/qld_higem_dblco2_precip_percentiles_range_combined.higem_stbl.'+season_name+'_range'+STRTRIM(STRING(ranges(i)),1)+'.ps'
      PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,SPACE2=2000,XOFFSET=500,YOFFSET=1500,TFONT=2,$
             TCHARSIZE=90,CB_WIDTH=100,/PORTRAIT,XSIZE=17000,SPACE3=500
      CS,SCALE=1,NCOLS=N_ELEMENTS(REFORM(mylevs_freq(*,i)))+1,/REV
      LEVS,MANUAL=REFORM(mylevs_freq(*,i))
      GSET,YMIN=0,YMAX=(ranges(i)*2+1)^2*(90.*90.)/(100.*100.),XMIN=0,XMAX=n_percentiles
      CON,Y=(indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100.),X=indgen(n_percentiles)+0.5,FIELD=REFORM(stbl_range_count(i,*,0:(ranges(i)*2+1)^2-1)),/BLOCK,$
      ...
      ;stbl_range_count[where(ctl_range_count le 0.02)]=!Values.F_NaN

      psfile='/home/ss901165/idl/queensland/higem/double_co2/precip_percentiles/qld_higem_dblco2_precip_percentiles_range_combined.higem_stbl-minus-higem_ctl.'+season_name+'_range'+STRTRIM(STRING(ranges(i)),1)+'.ps'
      PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,SPACE2=2000,XOFFSET=500,YOFFSET=1500,TFONT=2,$
             TCHARSIZE=90,CB_WIDTH=100,/PORTRAIT,XSIZE=17000,SPACE3=500
      CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs_diff)+1,/REV
      LEVS,MANUAL=mylevs_diff
      GSET,YMIN=0,YMAX=(ranges(i)*2+1)^2*(90.*90.)/(100.*100.),XMIN=0,XMAX=n_percentiles
      CON,Y=(indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100.),X=indgen(n_percentiles)+0.5,$
      ...
      psfile='/home/ss901165/idl/queensland/higem/double_co2/precip_percentiles/qld_higem_dblco2_precip_percentiles_range_combined.higem_stbl-ratio-higem_ctl.'+season_name+'_range'+STRTRIM(STRING(ranges(i)),1)+'.ps'
      PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,SPACE2=2000,XOFFSET=500,YOFFSET=1500,TFONT=2,$
             TCHARSIZE=90,CB_WIDTH=100,/PORTRAIT,XSIZE=17000,SPACE3=500
      CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs_ratio)+1,/REV
      LEVS,MANUAL=mylevs_ratio
      GSET,YMIN=0,YMAX=(ranges(i)*2+1)^2*(90.*90.)/(100.*100.),XMIN=0,XMAX=n_percentiles
      ratio=REFORM(stbl_range_count(i,*,0:(ranges(i)*2+1)^2-1))/REFORM(ctl_range_count(i,*,0:(ranges(i)*2+1)^2-1))
      ratio[where(ctl_range_count(i,*,0:(ranges(i)*2+1)^2-1) le 0.02)]=!Values.F_NaN
      CON,Y=(indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100.),X=indgen(n_percentiles)+0.5,$
          FIELD=ratio,/BLOCK,$
          TITLE='Ratio of size of daily precip events: 2xCO2 divided by CTL - '+season_name+' - Queensland',$
          CB_TITLE='Ratio of fractional frequencies',/NOLINES
      AXES,YVALS=(indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100),YLABELS=STRMID(STRTRIM(STRING((indgen((ranges(i)*2+1)^2)+1)*(90.*90.)/(100.*100)),1),0,4),$
           XVALS=indgen(n_percentiles)+0.5,XLABELS=STRMID(STRTRIM(STRING(percentiles),1),0,4),ORIENTATION=35,$
           YTITLE='Estimated area with precipitation >= percentile (10!U4!N km!U2!N)',$
           XTITLE='Percentile of daily rainfall for '+season_name+' (days >= 1 mm only) from CTL',NDECS=1
      PSCLOSE,/NOVIEW

      psfile='/home/ss901165/idl/queensland/higem/double_co2/precip_percentiles/qld_higem_dblco2_precip_percentiles_range_combined.higem_stbl-ratio-higem_ctl.'+season_name+'_range'+STRTRIM(STRING(ranges(i)),1)+'_mean.ps'
      PSOPEN,file=psfile,FONT=2,CHARSIZE=100,MARGIN=2500,SPACE2=2000,XOFFSET=500,YOFFSET=1500,TFONT=2,$
             TCHARSIZE=90,CB_WIDTH=100,SPACE3=500
      GSET,XMIN=0,XMAX=n_percentiles,YMIN=2,YMAX=15
      ctl_mean=fltarr(n_percentiles)
      stbl_mean=fltarr(n_percentiles)
      FOR k=0,n_percentiles-1 DO BEGIN
         ctl_mean(k)=TOTAL((indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100.)*ctl_range_count(i,k,0:(ranges(i)*2+1)^2-1))
         stbl_mean(k)=TOTAL((indgen((ranges(i)*2+1)^2)+0.5)*(90.*90.)/(100.*100.)*stbl_range_count(i,k,0:(ranges(i)*2+1)^2-1))*1.005^k
      ENDFOR
      GPLOT,X=indgen(n_percentiles)+0.5,Y=ctl_mean,COL=FSC_COLOR('black')
      GPLOT,X=indgen(n_percentiles)+0.5,Y=stbl_mean,COL=FSC_COLOR('red')
      AXES,YSTEP=1,YMINOR=0.5,$
           XVALS=indgen(n_percentiles)+0.5,XLABELS=STRMID(STRTRIM(STRING(percentiles),1),0,4),ORIENTATION=35,$
           YTITLE='Mean of estimated area with precipitation >= percentile (10!U4!N km!U2!N)',$
           XTITLE='Percentile of daily rainfall for '+season_name+' (days >= 1 mm only) from CTL',NDECS=1
      PSCLOSE,/NOVIEW
   ENDFOR
ENDFOR
