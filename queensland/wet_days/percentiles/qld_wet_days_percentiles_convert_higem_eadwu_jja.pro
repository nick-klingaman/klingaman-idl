PRO qld_wet_days_percentiles_convert_higem_eadwu_jja
...
eadwu_infile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eadwu.jun-aug_dmeans.o2-r3.precip.global_domain.nc.rearrange'
mask_infile='/home/ss901165/um_output/mask_n144_higam.nc'
eadwu_nyears=31
eadwu_ndays_per_year=90
...
eadwu_latitude=OPEN_AND_EXTRACT(eadwu_infile,'latitude')
eadwu_longitude=OPEN_AND_EXTRACT(eadwu_infile,'longitude')
DEFINE_BOUNDARIES,box,eadwu_latitude,eadwu_longitude,eadwu_box_tx,/LIMIT
eadwu_nlat=N_ELEMENTS(eadwu_latitude)
eadwu_nlon=N_ELEMENTS(eadwu_longitude)
...
outfile='/home/ss901165/higem_qccce/es_2xco2stbl_eadwu/higem_eadwu.jun-aug_dmeans.o2-r3.precip_percentiles.aus_domain.nc'
dimids=intarr(5)
varids=intarr(8)

id=NCDF_CREATE(outfile,/CLOBBER)
dimids(0)=NCDF_DIMDEF(id,'longitude',eadwu_nlon)
dimids(1)=NCDF_DIMDEF(id,'latitude',eadwu_nlat)
dimids(2)=NCDF_DIMDEF(id,'year',eadwu_nyears)
dimids(3)=NCDF_DIMDEF(id,'time',eadwu_ndays_per_year)
...
NCDF_VARPUT,id,varids(0),eadwu_longitude
NCDF_VARPUT,id,varids(1),eadwu_latitude
NCDF_VARPUT,id,varids(2),indgen(eadwu_nyears)+1958
NCDF_VARPUT,id,varids(3),indgen(eadwu_ndays_per_year)
NCDF_VARPUT,id,varids(4),percentiles

total_days=FLOAT(eadwu_nyears)*FLOAT(eadwu_ndays_per_year)
FOR i=0,eadwu_nlon-1 DO BEGIN
   FOR j=0,eadwu_nlat-1 DO BEGIN
      print,i,j,mask(i,j)
      thispt_amount_at_percentile=fltarr(n_percentiles)
;      thispt_precip=REFORM(OPEN_AND_EXTRACT(eadwu_infile,'rain',$
;                                            offset=[eadwu_box_tx(1)+i,eadwu_box_tx(0)+j,0,0],$
;                                            count=[1,1,eadwu_ndays_per_year,eadwu_nyears]),[total_days])
      print,'Reading ...'
      thispt_precip=REFORM(OPEN_AND_EXTRACT(eadwu_infile,'precip',$
                                            offset=[0,0,eadwu_box_tx(0)+j,eadwu_box_tx(1)+i],$
                                            count=[eadwu_nyears,eadwu_ndays_per_year,1,1]))*86400.
      print,mask(i,j),thispt_precip(0,0)
      print,'Reforming ...'
      thispt_precip_ts=fltarr(LONG(eadwu_nyears)*eadwu_ndays_per_year)
      FOR k=0,eadwu_nyears-1 DO $
         thispt_precip_ts(k*LONG(eadwu_ndays_per_year):(k+1)*LONG(eadwu_ndays_per_year)-1)=$
         thispt_precip(k,*)         
      print,'Computing percentiles ...'     
      wet_days=[where(thispt_precip_ts gt 1)]
      IF N_ELEMENTS(wet_days) gt 1 THEN BEGIN
         ...
         FOR k=0,eadwu_nyears-1 DO BEGIN
            lower_bound=LONG(k*FLOAT(eadwu_ndays_per_year))
            upper_bound=LONG((k+1)*FLOAT(eadwu_ndays_per_year)-1)            
;            NCDF_VARPUT,id,varids(5),thispt_precip_percentiles(lower_bound:upper_bound),$
;                        offset=[0,k,j,i],count=[eadwu_ndays_per_year,1,1,1]
            NCDF_VARPUT,id,varids(5),thispt_precip_percentiles(lower_bound:upper_bound),$
                        offset=[i,j,k,0],count=[1,1,1,eadwu_ndays_per_year]
         ENDFOR
