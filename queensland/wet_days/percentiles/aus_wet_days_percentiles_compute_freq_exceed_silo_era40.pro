PRO aus_wet_days_percentiles_compute_freq_exceed_silo_era40
...
input_dir='/home/ss901165/datasets_mango/SILO/era40_resolution'
percentiles_input_file=input_dir+'/SILO.mar-feb_dmeans.1958-2001.precip_percentiles.era40.nc'
...
FOR i=0,n_output_periods-1 DO BEGIN
   ...
   FOR j=0,n_lon-1 DO BEGIN
      FOR k=0,n_lat-1 DO BEGIN
         IF TOTAL(where(REFORM(percentiles(j,k,*,*)) lt 2e19) ne -1) THEN BEGIN
   ...
   outfile=input_dir+'/SILO.'+period_name+'_'+period_type+'s.1958-2001.precip_freq_exceed.era40.nc'
