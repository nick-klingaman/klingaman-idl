PRO qld_wet_days_percentiles_convert_ncep
...
ncep_infile='/home/ss901165/datasets_mango/NCEP_REANALYSIS/precipitation/ncep-ncar_reanalysis.mar-feb_dmeans.1958-2001.precip.t62_gauss.nc.rearrange'
mask_infile='/home/ss901165/datasets_mango/NCEP_REANALYSIS/lsmask.t62_gauss.nc'
ncep_nyears=44
ncep_ndays_per_year=365
...
ncep_latitude=OPEN_AND_EXTRACT(ncep_infile,'lat')
ncep_longitude=OPEN_AND_EXTRACT(ncep_infile,'lon')
DEFINE_BOUNDARIES,box,ncep_latitude,ncep_longitude,ncep_box_tx,/LIMIT
ncep_nlat=N_ELEMENTS(ncep_latitude)
ncep_nlon=N_ELEMENTS(ncep_longitude)
...
outfile='/home/ss901165/datasets_mango/NCEP_REANALYSIS/precipitation/ncep-ncar_reanalysis.mar-feb_dmeans.1958-2000.precip_percentiles.t62_gauss.nc'
dimids=intarr(5)
varids=intarr(8)

id=NCDF_CREATE(outfile,/CLOBBER)
dimids(0)=NCDF_DIMDEF(id,'longitude',ncep_nlon)
dimids(1)=NCDF_DIMDEF(id,'latitude',ncep_nlat)
dimids(2)=NCDF_DIMDEF(id,'year',ncep_nyears)
dimids(3)=NCDF_DIMDEF(id,'time',ncep_ndays_per_year)
...
NCDF_VARPUT,id,varids(0),ncep_longitude
NCDF_VARPUT,id,varids(1),ncep_latitude
NCDF_VARPUT,id,varids(2),indgen(ncep_nyears)+1958
NCDF_VARPUT,id,varids(3),indgen(ncep_ndays_per_year)
NCDF_VARPUT,id,varids(4),percentiles

total_days=FLOAT(ncep_nyears)*FLOAT(ncep_ndays_per_year)
FOR i=0,ncep_nlon-1 DO BEGIN
   FOR j=0,ncep_nlat-1 DO BEGIN
      print,i,j,mask(i,j)
      thispt_amount_at_percentile=fltarr(n_percentiles)
;      thispt_precip=REFORM(OPEN_AND_EXTRACT(ncep_infile,'rain',$
;                                            offset=[ncep_box_tx(1)+i,ncep_box_tx(0)+j,0,0],$
;                                            count=[1,1,ncep_ndays_per_year,ncep_nyears]),[total_days])
      print,'Reading ...'
      thispt_precip=(REFORM(OPEN_AND_EXTRACT(ncep_infile,'prate',$
                                            offset=[0,0,ncep_box_tx(0)+j,ncep_box_tx(1)+i],$
                                            count=[ncep_nyears,ncep_ndays_per_year,1,1]))*(1E-7)+0.0032765)*86400.
      print,mask(i,j),thispt_precip(0,0)
      IF mask(i,j) ne 0 and ABS(thispt_precip(0,0)) lt 1E6 THEN BEGIN
         print,'Reforming ...'
         thispt_precip_ts=fltarr(LONG(ncep_nyears)*ncep_ndays_per_year)
         FOR k=0,ncep_nyears-1 DO $
            thispt_precip_ts(k*LONG(ncep_ndays_per_year):(k+1)*LONG(ncep_ndays_per_year)-1)=$
            thispt_precip(k,*)         
         ...
         FOR k=0,ncep_nyears-1 DO BEGIN
            lower_bound=LONG(k*FLOAT(ncep_ndays_per_year))
            upper_bound=LONG((k+1)*FLOAT(ncep_ndays_per_year)-1)            
;            NCDF_VARPUT,id,varids(5),thispt_precip_percentiles(lower_bound:upper_bound),$
;                        offset=[0,k,j,i],count=[ncep_ndays_per_year,1,1,1]
            NCDF_VARPUT,id,varids(5),thispt_precip_percentiles(lower_bound:upper_bound),$
                        offset=[i,j,k,0],count=[1,1,1,ncep_ndays_per_year]
         ENDFOR
