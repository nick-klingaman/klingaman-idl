PRO qld_wet_days_percentiles_freqtrend
...
start_date=[1,12,1901]          ; 1 December
stop_date=[31,3,1901]           ; 31 March
...
CS,SCALE=26,NCOLS=N_ELEMENTS(mylevs)+1,/REV,white=[10]
...
LEVS,MANUAL=mylevs
CON,FIELD=exceedance_frequency_trend,Y=latitude,X=longitude,$
    TITLE='Linear trend in frequency of exceeding climatological '+STRTRIM(STRING(percentile),1)+$
    ' percentile of daily rainfall - Dec-Mar - SILO 1900-2008',/NOLINES,/BLOCK,$
    CB_TITLE='Days year!U-2!N exceeding threshold'
FOR i=0,n_lon-1 DO $
   FOR j=0,n_lat-1 DO $
      IF (ABS(exceedance_frequency_corr(i,j)) gt sig_level) THEN $
