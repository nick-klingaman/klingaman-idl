PRO qld_wet_days_percentiles_freqtrend_scatter_region_95
...
;nino4_anomalies_filter=nino4_anomalies(where(exceedance_frequency_area_avg lt 1))
nino4_anomalies=nino4_ssts-MEAN(nino4_ssts)
nino4_anomalies_filter=nino4_anomalies[where(exceedance_frequency_area_avg gt 0.01)]
en_anomalies=nino4_anomalies_filter[where(nino4_anomalies_filter gt 0.3)]
ln_anomalies=nino4_anomalies_filter[where(nino4_anomalies_filter lt -0.3)]

exceedance_frequency_area_avg_en=exceedance_frequency_area_avg(where(nino4_anomalies gt 0.3 and exceedance_frequency_area_avg gt 0.01))
exceedance_frequency_area_avg_ln=exceedance_frequency_area_avg(where(nino4_anomalies lt -0.3 and exceedance_frequency_area_avg gt 0.01))
...
psfile='/home/swu07adk/silo/qld_wet_days_percentiles_nion4.sep-nov_scatter_reg3_95.ps'
;psfile='/home/ss901165/idl/queensland/temp.ps'
...
GSET,XMIN=-2,XMAX=2,YMIN=0,YMAX=2.5,TITLE='Scatter-plot of NINO4 SST anomalies and exceedance frequency at the 95th percentile in Region 3 (Sep-Nov)'
GPLOT,X=xpts,Y=ypts,SYM=2,/NOLINES
coefficients_en=LINFIT(xpts_en,ypts_en,yfit=yfit_en)
coefficients_ln=LINFIT(xpts_ln,ypts_ln,yfit=yfit_ln)
GPLOT,X=xpts_en,Y=yfit_en,COL=FSC_COLOR("red")
GPLOT,X=xpts_ln,Y=yfit_ln,COL=FSC_COLOR("blue")
GPLOT,X=-1,Y=2.0,TEXT=correlation_ln,COL=FSC_COLOR("blue")
GPLOT,X=1,Y=2.0,TEXT=correlation_en,COL=FSC_COLOR("red")
GPLOT,X=[0,0],Y=[0,2.5],STYLE=1
AXES,XTITLE='NINO4 SST anomalies (K)',YTITLE='Exceedance frequency at the 95th percentile (Days)',NDECS=2
PSCLOSE
