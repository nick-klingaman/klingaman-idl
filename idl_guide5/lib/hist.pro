...
IF KEYWORD_SET(TOWER) THEN BEGIN
 ...
 IF KEYWORD_SET(VERTICAL) THEN BEGIN
  FOR itower=0, s(2)-1 DO BEGIN
   ...
   HIST, X=x, Y=Y(*, itower), FILLCOL=fillcol(itower), WIDTH=width, OFFSET=offset, NOBORDER=noborder, THICK=thick 
  ENDFOR
 ENDIF
 IF KEYWORD_SET(HORIZONTAL) THEN BEGIN
  ...
  FOR itower=0, s(2)-1 DO BEGIN
   ...
   HIST, X=x(*, itower), Y=y, FILLCOL=fillcol(itower), WIDTH=width, OFFSET=offset, NOBORDER=noborder, THICK=thick , /HORIZONTAL
  ENDFOR
 ENDIF 
ENDIF
