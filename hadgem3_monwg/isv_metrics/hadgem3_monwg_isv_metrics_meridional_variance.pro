PRO hadgem3_monwg_isv_metrics_meridional_variance
...
FOR i=0,n_sets-1 DO BEGIN
    ...
    allyears_precip_filtered_lonavg=fltarr(n_lat,n_days*n_years)
    allyears_precip_unfiltered_lonavg=fltarr(n_lat,n_days*n_years)
    FOR j=0,n_years-1 DO BEGIN
       ...
       FOR k=0,n_lat-1 DO BEGIN
          FOR m=0,n_days-1 DO BEGIN
             allyears_precip_filtered_lonavg(k,j*n_days+m)=MEAN(thisyear_precip_filtered(*,k,m),/NaN)
             IF FINITE(allyears_precip_filtered_lonavg(k,j*n_days+m)) eq 0 THEN $
                allyears_precip_filtered_lonavg(k,j*n_days+m)=allyears_precip_filtered_lonavg(k,j*n_days+m-1)
             allyears_precip_unfiltered_lonavg(k,j*n_days+m)=MEAN(thisyear_precip_unfiltered(*,k,m),/NaN)
             IF FINITE(allyears_precip_unfiltered_lonavg(k,j*n_days+m)) eq 0 THEN $
                allyears_precip_unfiltered_lonavg(k,j*n_days+m)=allyears_precip_unfiltered_lonavg(k,j*n_days+m-1)
          ENDFOR
       ENDFOR
    ENDFOR
    
    precip_filtered_lonavg_variance=fltarr(n_lat)
    precip_unfiltered_lonavg_variance=fltarr(n_lat)
    FOR j=0,n_lat-1 DO BEGIN
        precip_filtered_lonavg_variance(j)=VARIANCE(allyears_precip_filtered_lonavg(j,*))
        precip_unfiltered_lonavg_variance(j)=VARIANCE(allyears_precip_unfiltered_lonavg(j,*))
    ENDFOR

    IF i eq 0 THEN BEGIN
        psfile='/home/ss901165/idl/hadgem3_monwg/isv_metrics/hadgem3_monwg_isv_metrics_meridional_variance.filter'+low_limit_str+high_limit_str+'.'+region_name+'.ps'
    ...
    GSET,XMIN=lat_range(0),XMAX=lat_range(1),YMIN=0,YMAX=4,$
         TITLE="For rain lon-avg "+STRTRIM(STRING(lonavg_range(0)),1)+"E-"+STRTRIM(STRING(lonavg_range(1)),1)+$
         "E, std. dev. in "+low_limit_str+'-'+high_limit_str+" day bp-filtered rain and %age of 2-120 day var explained"
    AXES,XSTEP=5,XTITLE="Latitude (degrees north)",/NOLEFT,/NORIGHT
    AXES,YSTEP=0.5,YTITLE="Std. dev. in 10-20 day filtered rainfall (lon. avg. 115-135E)",/ONLYLEFT,NDECS=2

    color=FSC_COLOR(color_names(i),color_numbers(i))
    GPLOT,Y=SQRT(precip_filtered_lonavg_variance),X=latitude,THICK=200,COL=color_numbers(i)
    GSET,XMIN=lat_range(0),XMAX=lat_range(1),YMIN=0,YMAX=40
    AXES,YSTEP=5,YTITLE="Percentage of subseasonal (2-120 day) variance explained",/ONLYRIGHT
    GPLOT,Y=precip_filtered_lonavg_variance/precip_unfiltered_lonavg_variance*100.,X=latitude,THICK=200,$
