PRO hadgem3_monwg_isv_metrics_probabilities_glosea4_wetwet
...
FOR i=0,n_years-1 DO BEGIN
   ...
   IF i eq 0 THEN BEGIN
      psfile='/home/ss901165/idl/hadgem3_monwg/isv_metrics/hadgem3_monwg_isv_metrics_probabilities_glosea4_wetwet.'+model_start_date+'.'+model_ens_member+'.ps'
        ...
        GSET,XMIN=MIN(heaviside),XMAX=MAX(heaviside),YMIN=0,YMAX=1,$
          TITLE='India - prob of rain|rain for model '+model_start_date+'-'+model_ens_member+' (solid) and TRMM (dashed)'+$
             ' and prob of dry for model (cross) and TRMM (triangle)'        
    ...
    GPLOT,Y=REFORM(points_rain_given_rain(0,*))/REFORM(points_rain(0,*)),X=heaviside,THICK=200,COL=color_numbers(i),STYLE=0
    GPLOT,Y=REFORM(points_rain_given_rain(1,*))/REFORM(points_rain(1,*)),X=heaviside,THICK=200,COL=color_numbers(i),STYLE=2
;    GPLOT,Y=points_not_rain_given_not_rain/points_not_rain,X=heaviside,THICK=200,COL=color_numbers(i),STYLE=1
    ...
    GPLOT,Y=REFORM(points_not_rain(0,*))/points,X=heaviside,/NOLINES,SYM=6,COL=color_numbers(i)
    GPLOT,Y=REFORM(points_not_rain(1,*))/points,X=heaviside,/NOLINES,SYM=4,COL=color_numbers(i)
...
GLEGEND,labels=REVERSE(['Model','TRMM']),SYM=REVERSE([6,4]),LEGXOFFSET=-11000,LEGYOFFSET=3500,LENGTH=1
GLEGEND,labels=REVERSE(['Model','TRMM']),STYLE=REVERSE([0,2]),LEGXOFFSET=-11000,LEGYOFFSET=5000
