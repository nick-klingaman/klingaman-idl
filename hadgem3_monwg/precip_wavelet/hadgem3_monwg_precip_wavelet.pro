PRO hadgem3_monwg_precip_wavelet

; Plot wavelet transforms of precipitation for the HadGEM3-AO run ahsaf.
...
; is at or above the 90% confidence interval (vs. red noise) during
; the monsoon season.  Output this PDF to a netCDF file.


; Input file containing all months, all years
hadgem3_infile='/home/ss901165/um_output2/hadgem3_monwg/ahsaf/ahsaf.precip.daily_20years.nc'
...
precip_aavg=fltarr(n_days)
thisyear_precip_aavg=fltarr(n_days_per_year)
...
FOR i=0,n_years-1 DO BEGIN
    ...
    thisyear_precip=REFORM(OPEN_AND_EXTRACT(hadgem3_infile,'precip',$
                                     offset=[hadgem3_box_tx(1),hadgem3_box_tx(0),0,0,i],$
                                     count=[hadgem3_nlon,hadgem3_nlat,1,n_days_per_year,1]))*86400.
    FOR j=0,n_days_per_year-1 DO $
      thisyear_precip_aavg(j)=MEAN(thisyear_precip(*,*,j))
    precip_aavg(i*n_days_per_year:(i+1)*n_days_per_year-1)=thisyear_precip_aavg
ENDFOR

print,'Now computing wavelet for all years and all months ...'
wave = WAVELET(precip_aavg,1,/pad,lag1=A_CORRELATE(precip_aavg,2),siglvl=0.90,signif=sig90,$
               period=period,scale=scale)
wave = WAVELET(precip_aavg,1,/pad,lag1=A_CORRELATE(precip_aavg,2),siglvl=0.95,signif=sig95,$
...
mylevs=['16','32','64','128','256','512','1024','2048']
...
FOR i=0,n_years-1 DO BEGIN
    ...
    ;psfile='/home/ss901165/idl/hadgem3_monwg/precip_wavelet/hadgem3_monwg_precip_wavelet.ahsaf.year'+$
...
psfile='/home/ss901165/idl/hadgem3_monwg/precip_wavelet/hadgem3_monwg_precip_wavelet.ahsaf_mean.NBoB.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=150,MARGIN=1500,SPACE2=300,XOFFSET=500,YOFFSET=200,TFONT=2,TCHARSIZE=110
CS,IDL=3,NCOLS=N_ELEMENTS(mylevs)+1,/REV
GSET,XMIN=offset_may1,XMAX=offset_oct30,YMIN=150,YMAX=MIN(period),/YLOG
LEVS,MANUAL=mylevs
CON,FIELD=power_ensmean,X=indgen(n_days_per_year),Y=period,$
  TITLE="Mean wavelet transform of rainfall in the northern BoB (15-20N, 85-90E) for HadGEM3-AO_ahsaf_n96 (20 years)",/NOLINES
LEVS,MANUAL=['0','95']
white=FSC_COLOR("white",50)
...
out_id=NCDF_CREATE('/home/ss901165/idl/hadgem3_monwg/precip_wavelet/hadgem3_monwg_isv_power_pdf.ahsaf.NBoB.nc',/CLOBBER)
...
NCDF_ATTPUT,out_id,out_varids(1),'long_name','Probability distribution function for power of 30-50 day variability of precipitation in the northern Bay of Bengal'
