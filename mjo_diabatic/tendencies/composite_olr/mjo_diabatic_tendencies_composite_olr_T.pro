PRO mjo_diabatic_tendencies_composite_olr_T,model_names,start_date,stop_date,box,region_name,lead_times=lead_times
...
FOR i=0,n_models-1 DO BEGIN
   ...
   CASE model_names(i) OF
      'ecmwf' : BEGIN
         ...
         level_name='level'
         increment_names=['tnt','tnta','tntc','tntd','tntlscp','tntpbl','tntrlw','tntrsw']
         increment_descs=['Total','Advection','Convection','Other','LS cld+prcp','B-layer','Longwave','Shortwave']
         increment_colors=['black','purple','red','dodgerblue','violetred','blue','brown','orange']
         multiplier=86400.
         precip_multiplier=1.
         level_multiplier=1.
         missing_value=1e10
      ...
      'miroc' : BEGIN
         ...
         latitude_name='latitude'
         level_name='level'
         increment_names=['tnt','tnta','tntc','tntlscp','tntpbl','tntrlw','tntrsw']
         increment_descs=['Total','Advection','Convection','LS cld+prcp','B-layer','Longwave','Shortwave']
         increment_colors=['black','purple','red','violetred','blue','brown','orange']
         multiplier=86400.
         precip_multiplier=86400.
         level_multiplier=1.
         missing_value=1000
      ...
      'cancm4' : BEGIN
         ...
         latitude_name='latitude'
         level_name='level'
         increment_names=['tnt','tnta','tntc','tntd','tntscpbl','tntrlw','tntrsw']
         increment_descs=['Total','Advection','Convection','Other','LScld+prcp + Blyr','Longwave','Shortwave']
         increment_colors=['black','purple','red','dodgerblue','blue','brown','orange']
         multiplier=86400.
         precip_multiplier=86400.
         level_multiplier=0.01
         missing_value=1e20
      ...
      'spcam' : BEGIN
         ...
         latitude_name='latitude'
         level_name='lev_p'
         increment_names=['tnt','tnta','tntsp','tntpbl','tntrlw','tntrsw']
         increment_descs=['Total','Advection','CRM','KE diss','Longwave','Shortwave']
         increment_colors=['black','purple','red','blue','brown','orange']
         multiplier=86400.
         precip_multiplier=86400.
         level_multiplier=1.
         missing_value=1e20
      ...
      'metum' : BEGIN
         ...
         latitude_name='latitude'
         level_name='level'
         increment_names=['tnt','tnta','tntc','tntd','tntlscld','tntlsp','tntpbl','tntrlw','tntrsw']
         increment_descs=['Total','Advection','Convection','Other','LS cld','LS prcp','B-layer','Longwave','Shortwave']
         increment_colors=['black','purple','red','dodgerblue','violetred','pink','blue','brown','orange']
         multiplier=72.
         precip_multiplier=86400.
         level_multiplier=1.
         missing_value=1e6
      ...
      'nicam' : BEGIN
         ...
         latitude_name='latitude'
         level_name='level'
         increment_names=['tnt','tnta','tntcmp','tntpbl','tntrlw','tntrsw']
         increment_descs=['Total','Advection','Microphys','B-layer','Longwave','Shortwave']
         increment_colors=['black','purple','red','blue','brown','orange']
         multiplier=86400.
         precip_multiplier=86400.
         level_multiplier=1.
...
;                  ' using hindcast initialised on '+initial_date
            ...
            IF grid_flag eq 0 THEN BEGIN
               model_longitude=OPEN_AND_EXTRACT(model_rlut_infile(0),longitude_name)
               model_latitude=OPEN_AND_EXTRACT(model_rlut_infile(0),latitude_name)
   ...
   FOR m=0,n_increments-1 DO BEGIN
      ...
      FOR k=0,n_lead_times-1 DO BEGIN
         ...
         total_transition_active_points=0
         total_transition_suppressed_points=0
         total_active_precip=0
         total_suppressed_precip=0
         total_transactive_precip=0
         total_transsuppressed_precip=0
         FOR n=0,model_nlon-1 DO BEGIN
            FOR p=0,model_nlat-1 DO BEGIN
               FOR q=0,n_times_per_day-1 DO BEGIN
                  ...
                  this_pt_pr=REFORM(model_precip(n,p,q:n_days*n_times_per_day-1:n_times_per_day,k))
                  active_points=where(this_pt_olr le model_olr_mean(n,p,q,k)-model_olr_stddev(n,p,q,k))
                  total_active_points=total_active_points+N_ELEMENTS(active_points)
                  suppressed_points=where(this_pt_olr ge model_olr_mean(n,p,q,k)+model_olr_stddev(n,p,q,k))
                  total_suppressed_points=total_suppressed_points+N_ELEMENTS(suppressed_points)
                  transition_active_points=where(this_pt_olr lt model_olr_mean(n,p,q,k)-model_olr_stddev(n,p,q,k)/2. and $
                                                 this_pt_olr gt model_olr_mean(n,p,q,k)-model_olr_stddev(n,p,q,k))
                  transition_suppressed_points=where(this_pt_olr gt model_olr_mean(n,p,q,k)+model_olr_stddev(n,p,q,k)/2. and $
                                                     this_pt_olr lt model_olr_mean(n,p,q,k)+model_olr_stddev(n,p,q,k))
                  total_transition_active_points=total_transition_active_points+N_ELEMENTS(transition_active_points)
                  total_transition_suppressed_points=total_transition_suppressed_points+N_ELEMENTS(transition_suppressed_points)
                  FOR r=0,model_nz-1 DO BEGIN
                     this_pt_inc=REFORM(this_increment(n,p,r,q:n_days*n_times_per_day-1:n_times_per_day,k))
                     IF TOTAL(active_points) ne -1 THEN $ 
                        model_increments_active(m,r,k)=model_increments_active(m,r,k)+TOTAL(this_pt_inc[active_points],/NaN) ;/TOTAL(this_pt_pr[active_points])
                     IF TOTAL(suppressed_points) ne -1 THEN $
                        model_increments_suppressed(m,r,k)=model_increments_suppressed(m,r,k)+TOTAL(this_pt_inc[suppressed_points],/NaN) ;/this_pt_pr[suppressed_points])
                     IF TOTAL(transition_active_points) ne -1 THEN $
                        model_increments_transactive(m,r,k)=model_increments_transactive(m,r,k)+$
                                                            TOTAL(this_pt_inc[transition_active_points],/NaN) ;/this_pt_pr[transition_active_points])
                     IF TOTAL(transition_suppressed_points) ne -1 THEN $
                        ...
                        TOTAL(this_pt_inc[transition_suppressed_points],/NaN) ;/this_pt_pr[transition_suppressed_points])                                          
                  ENDFOR
                  IF TOTAL(active_points) ne -1 THEN $
                     total_active_precip=total_active_precip+TOTAL(this_pt_pr[active_points])
                  IF TOTAL(suppressed_points) ne -1 THEN $
                     total_suppressed_precip=total_suppressed_precip+TOTAL(this_pt_pr[suppressed_points])
                  IF TOTAL(transition_active_points) ne -1 THEN $
                     total_transactive_precip=total_transactive_precip+TOTAL(this_pt_pr[transition_active_points])
                  IF TOTAL(transition_suppressed_points) ne -1 THEN $
                     total_transsuppressed_precip=total_transsuppressed_precip+TOTAL(this_pt_pr[transition_suppressed_points])
               ENDFOR
            ENDFOR
         ENDFOR
         ...
         model_increments_active(m,*,k)=model_increments_active(m,*,k)/FLOAT(total_active_precip)
         model_increments_suppressed(m,*,k)=model_increments_suppressed(m,*,k)/FLOAT(total_suppressed_precip)
         model_increments_transactive(m,*,k)=model_increments_transactive(m,*,k)/FLOAT(total_transactive_precip)
         model_increments_transsuppressed(m,*,k)=model_increments_transsuppressed(m,*,k)/FLOAT(total_transsuppressed_precip)
      ENDFOR
   ENDFOR
   FOR k=0,n_lead_times-1 DO BEGIN      
      psfile='/home/ss901165/idl/mjo_diabatic/tendencies/composite_olr/mjo_diabatic_tendencies_composite_olr_T.by_stddev.'+$
      ...
      GSET,XMIN=-1,XMAX=1,YMIN=MAX(model_z)+25,YMAX=MIN(model_z),TITLE=plot_desc+' dq/dt '+region_name+$
           ' active (OLR < mean-sdev) at '+STRTRIM(STRING(our_lead_times(k)),1)+' day(s) - '+start_date+' to '+stop_date
      ...
      AXES,XSTEP=0.1,XMINOR=0.05,YVALS=FLOOR(model_z+0.01),YTITLE='Pressure (hPa)',XTITLE='Heating rate (K mm!U-1!N)',NDECS=1,$
      ...
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=1500,SPACE3=200,XOFFSET=2000,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=110,SPACE2=2000,/PORTRAIT
      GSET,XMIN=-4,XMAX=4,YMIN=MAX(model_z)+25,YMAX=MIN(model_z),TITLE=plot_desc+' dq/dt '+region_name+$
           ' suppress (OLR > mean+sdev) at '+STRTRIM(STRING(our_lead_times(k)),1)+' day(s) - '+start_date+' to '+stop_date
      FOR m=0,n_increments-1 DO $
         GPLOT,X=REFORM(model_increments_suppressed(m,*,k)),Y=model_z,COL=FSC_COLOR(increment_colors(m))
      GPLOT,X=[0,0],Y=[MIN(model_z),MAX(model_z)+25],COL=FSC_COLOR('black'),STYLE=1
      GLEGEND,labels=REVERSE(increment_descs),COL=REVERSE(FSC_COLOR(increment_colors)),LEGXOFFSET=2000,LEGYOFFSET=26000
      AXES,XSTEP=0.4,XMINOR=0.2,YVALS=FLOOR(model_z+0.01),YTITLE='Pressure (hPa)',XTITLE='Heating rate (K day!U-1!N)',NDECS=1,$
           /NORIGHT,/NOUPPER,ORIENTATION=30
      PSCLOSE,/NOVIEW
      ...
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=1500,SPACE3=200,XOFFSET=2000,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=110,SPACE2=2000,/PORTRAIT
      GSET,XMIN=-1,XMAX=1,YMIN=MAX(model_z)+25,YMAX=MIN(model_z),TITLE=plot_desc+' dq/dt '+region_name+$
           ' transact (mn-sd<OLR<mn-sd/2) at '+STRTRIM(STRING(our_lead_times(k)),1)+' day(s) - '+start_date+' to '+stop_date
      FOR m=0,n_increments-1 DO $
         GPLOT,X=REFORM(model_increments_transactive(m,*,k)),Y=model_z,COL=FSC_COLOR(increment_colors(m))
      GPLOT,X=[0,0],Y=[MIN(model_z),MAX(model_z)+25],COL=FSC_COLOR('black'),STYLE=1
      GLEGEND,labels=REVERSE(increment_descs),COL=REVERSE(FSC_COLOR(increment_colors)),LEGXOFFSET=2000,LEGYOFFSET=26000
      AXES,XSTEP=0.1,XMINOR=0.05,YVALS=FLOOR(model_z+0.01),YTITLE='Pressure (hPa)',XTITLE='Heating rate (K day!U-1!N)',NDECS=1,$
           /NORIGHT,/NOUPPER,ORIENTATION=30
      PSCLOSE,/NOVIEW
      ...
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=1500,SPACE3=200,XOFFSET=2000,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=110,SPACE2=2000,/PORTRAIT
      GSET,XMIN=-3,XMAX=3,YMIN=MAX(model_z)+25,YMAX=MIN(model_z),TITLE=plot_desc+' dq/dt '+region_name+$
           ' transsup (mn+sd/2<OLR<mn+sd) at '+STRTRIM(STRING(our_lead_times(k)),1)+' day(s) - '+start_date+' to '+stop_date
      FOR m=0,n_increments-1 DO $
         GPLOT,X=REFORM(model_increments_transsuppressed(m,*,k)),Y=model_z,COL=FSC_COLOR(increment_colors(m))
      GPLOT,X=[0,0],Y=[MIN(model_z),MAX(model_z)+25],COL=FSC_COLOR('black'),STYLE=1
      GLEGEND,labels=REVERSE(increment_descs),COL=REVERSE(FSC_COLOR(increment_colors)),LEGXOFFSET=2000,LEGYOFFSET=26000
      AXES,XSTEP=0.3,XMINOR=0.15,YVALS=FLOOR(model_z+0.01),YTITLE='Pressure (hPa)',XTITLE='Heating rate (K day!U-1!N)',NDECS=1,$
           /NORIGHT,/NOUPPER,ORIENTATION=30
      PSCLOSE,/NOVIEW
