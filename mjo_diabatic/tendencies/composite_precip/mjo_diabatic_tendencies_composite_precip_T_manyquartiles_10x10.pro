PRO mjo_diabatic_tendencies_composite_precip_T_manyquartiles_10x10,model_names=model_names,inc_type,start_date,stop_date,box,region_name,lead_times=lead_times,plot_yotc=plot_yotc
...
ENDIF ELSE $
   our_model_names=['ecmwf','mri','miroc','nasa','nrl','spcam','nicam','giss','cancm4','cnrm_atmos','ecearth','cam5zm','metum'];,'metum_highentrain']
...
metum_valid_dates=standard_valid_dates[where(standard_valid_dates ne '20091106' and $
                                             standard_valid_dates ne '20091018' and $
                                             standard_valid_dates ne '20091021' and $
                                             standard_valid_dates ne '20091028' and $
                                             standard_valid_dates ne '20091123' and $
                                             standard_valid_dates ne '20100111')]
...
IF KEYWORD_SET(plot_yotc) THEN BEGIN
   ...
   model_increments_frachigh=fltarr(n_models+1,5)
   total_precip=fltarr(n_models+1,5,4)
ENDIF ELSE BEGIN
   model_increments=fltarr(n_models,5,4,model_nz) ;,n_lead_times)
   model_increments_frachigh=fltarr(n_models,5)
   total_precip=fltarr(n_models,5,4)
ENDELSE

; Pressure level to use to compute fraction of total heating above/below
p_frachigh=530
...
IF KEYWORD_SET(plot_yotc) THEN BEGIN
   ...
   CASE inc_type OF
      'physics' : BEGIN
         increment_name='tnt_physics'
         inc_file=yotc_dir+'/ECMWF_YOTC.oct2009-feb2010_3hrmeans.tnt_physics.10x10.nc'
         xmin=[-2,-4,-4,-4,-4]
         xmax=[10.01,4,6,6,6]
      ...
      'advection' : BEGIN
         increment_name='tnta'
         inc_file=yotc_dir+'/ECMWF_YOTC.oct2009-feb2010_3hrmeans.tnta.10x10.nc'
         xmin=[-14,-4,-8,-6,-6]
         xmax=[2,3,4,4,4]
      ...
      'radiation' : BEGIN
         increment_name='tntr'
         inc_file=yotc_dir+'/ECMWF_YOTC.oct2009-feb2010_3hrmeans.tntr.10x10.nc'
         xmin=[-3]
         xmax=[1]
         xstep=[0.5]
         legxoffset=-7000
      ...
      'all' : BEGIN
         increment_name='tnt'
         inc_file=yotc_dir+'/ECMWF_YOTC.oct2009-feb2010_3hrmeans.tnt.10x10.nc'
         xmin=[-1.6,-2,-2,-1,-1]
         xmax=[1.6,2,2,1,1]
   ...
   frachigh_lev=NEAREST(z,p_frachigh)
   low=0
   high=frachigh_lev   
   FOR j=0,4 DO BEGIN     
      p_total=0
      FOR p=low,high DO $
         model_increments_frachigh(n_models,j)=model_increments_frachigh(n_models,j)+model_increments(n_models,j,0,p)*(z(p)-z(p+1))
      temp=0
      p_total=0
      FOR p=0,nz-2 DO BEGIN
         temp=temp+model_increments(n_models,j,0,p)*(z(p)-z(p+1))
         p_total=p_total+(z(p)-z(p+1))
      ENDFOR
      model_increments_frachigh(n_models,j)=(model_increments_frachigh(n_models,j)/p_total)/(temp/p_total)
   ENDFOR
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/tendencies/composite_precip/mjo_diabatic_tendencies_composite_precip_T_manyquartiles_10x10.'+$
...
;           STYLE=REVERSE([0,1,2,3]),LEGPOS=legpos(0),SYM=REVERSE(syms)
   AXES,XSTEP=xstep(0),XMINOR=xstep(0)/2.,YVALS=FLOOR(z+0.01),YTITLE='Pressure (hPa)',$
        XTITLE='Heating rate (K day!U-1!N)',NDECS=1,/NORIGHT,/NOUPPER,ORIENTATION=30   
;   FOR j=0,3 DO $
...
FOR i=0,n_models-1 DO BEGIN
   ...
   CASE our_model_names(i) OF
      ...
      'metum_highentrain' : BEGIN
         ...
         file_descs(i)='MetUM_highentrain'
         model_names(i)='ME'
         model_colors(i)='steelblue'
...
;               model_nz=N_ELEMENTS(model_z)
         ...
         ENDFOR
      ENDIF ELSE IF grid_flag eq 1 THEN $
         FOR k=0,n_lead_times-1 DO $
            model_precip(*,*,j*n_times_per_day:(j+1)*n_times_per_day-1,k)=!Values.F_NaN
   ENDFOR
   IF TOTAL(where(ABS(model_precip) ge ABS(missing_value))) ge 0 and grid_flag eq 1 THEN $
      ...
      IF TOTAL(where(valid_dates eq dates(j+start_position(0)))) ne -1 THEN BEGIN     
         FOR k=0,n_lead_times-1 DO BEGIN    
            ...
            IF grid_flag eq 0 THEN BEGIN
               ...
               frachigh_lev=NEAREST(model_z,p_frachigh)
...
;   total_nearzero_points=0
   ...
   IF reverse_levels(i) eq 1 THEN BEGIN
      low=0
      high=frachigh_lev
   ENDIF ELSE BEGIN
      low=frachigh_lev
   ...
   FOR j=0,4 DO BEGIN     
      p_total=0
      FOR p=low,high DO $
         model_increments_frachigh(i,j)=model_increments_frachigh(i,j)+model_increments(i,j,0,p)*(model_z(p)-model_z(p+1))
      temp=0
      p_total=0
      FOR p=0,model_nz-2 DO BEGIN
         temp=temp+model_increments(i,j,0,p)*(model_z(p)-model_z(p+1))
         p_total=p_total+(model_z(p)-model_z(p+1))
      ENDFOR
      model_increments_frachigh(i,j)=(model_increments_frachigh(i,j)/p_total)/(temp/p_total)
   ENDFOR
...
FOR m=0,n_models-1 DO BEGIN
   ...
   PSOPEN,file=psfile,FONT=2,CHARSIZE=180,MARGIN=3000,SPACE3=200,XOFFSET=500,YOFFSET=500,TFONT=2,TCHARSIZE=90,$
          CB_WIDTH=110,SPACE2=2000,/PORTRAIT
   GSET,XMIN=xmin(0),XMAX=xmax(0),YMIN=MAX(model_z)+25,YMAX=MIN(model_z) ;,TITLE=file_descs(m)+' '+inc_type+' dT/dt '+region_name+$
                                ;' at '+STRTRIM(STRING(MIN(our_lead_times)),1)+'-'+STRTRIM(STRING(MAX(our_lead_times)),1)+' day(s) - '+start_date+' to '+stop_date     
   FOR j=0,4 DO BEGIN
      FOR k=0,0 DO BEGIN
         IF file_descs(m) eq 'ECMWF_IFS' or file_descs(m) eq 'CanCM4' THEN BEGIN            
            GPLOT,X=REFORM(model_increments(m,j,k,1:model_nz-1)),Y=model_z(1:model_nz-1),COL=FSC_COLOR(colors(j)),STYLE=styles(k),THICK=200
         ENDIF ELSE BEGIN
            IF reverse_levels(m) eq 0 THEN BEGIN
...
;           STYLE=REVERSE([0,1,2,3]),LEGPOS=legpos(0),SYM=REVERSE(syms)
   AXES,XSTEP=xstep(0),XMINOR=xstep(0)/2.,YVALS=FLOOR(model_z+0.01),YTITLE='Pressure (hPa)',$
        XTITLE='Heating rate (K day!U-1!N)',NDECS=1,/NORIGHT,/NOUPPER,ORIENTATION=30   
;   FOR j=0,3 DO $
;      GPLOT,X=xmax(0)*0.9,Y=350-25*j,TEXT=STRMID(STRTRIM(STRING(model_increments_frachigh(m,j)),1),0,4),COL=FSC_COLOR(colors(j))
;   GPLOT,X=xmax(0)*0.9,Y=375,TEXT=STRMID(STRTRIM(STRING(model_increments_frachigh(m,4)),1),0,4),COL=FSC_COLOR(colors(4))
