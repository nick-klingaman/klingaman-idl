PRO mjo_diabatic_plot_qadv_Tadv_vint,model_names=model_names,start_date,stop_date,lead_times,box,box_name,mask_type,plot_yotc=plot_yotc
...
;For physics
tadv_bins=findgen(21)*1-12.5
qadv_bins=findgen(21)*1-7.5
;qadv_bins=findgen(28)*0.2-3.9
;tadv_bins=findgen(24)*0.2-2.3
;qadv_bins=findgen(24)*0.2-2.3
...
FOR i=0,n_models-1 DO BEGIN
   ...
   CASE our_model_names(i) OF
      'ecmwf' : BEGIN
         ...
         qadv_multiplier=86400.*1000.*2.5
         level_multiplier=1.
         reverse_level=0
         missing_value=1e10
         has_landsea=1
      ...
      'giss' : BEGIN
         ...
         level_name='level'
         tadv_multiplier=86400.
         qadv_multiplier=1000.*86400.*2.5
         level_multiplier=1.
         reverse_level=0
         missing_value=1e5
         has_landsea=0
...
;      print,TOTAL(where(valid_dates eq standard_valid_dates(j+start_position(0))))
      IF TOTAL(where(valid_dates eq standard_valid_dates(j+start_position(0)))) ne -1 THEN BEGIN     
         FOR k=0,n_lead_times-1 DO BEGIN    
            ...
            ;print,tadv_infile
            qadv_infile=indir+'/'+REFORM(initial_date)+'/'+file_desc+'.tnhusa.'+REFORM(initial_date)+'.00Z.nc'
            IF grid_flag eq 0 THEN BEGIN
               ...
               qadv_longitude=OPEN_AND_EXTRACT(qadv_infile(0),longitude_name)
               qadv_latitude=OPEN_AND_EXTRACT(qadv_infile(0),latitude_name)
               DEFINE_BOUNDARIES,box,qadv_latitude,qadv_longitude,qadv_box_tx,/LIMIT
               qadv_nlon=N_ELEMENTS(qadv_longitude)
               qadv_nlat=N_ELEMENTS(qadv_latitude)
               ...
               mean_qadvbin=fltarr(n_qadvbins+1)
               median_tadvbin=fltarr(n_tadvbins+1)
               median_qadvbin=fltarr(n_qadvbins+1)
               tadv_anom_vint=fltarr(tadv_nlon,tadv_nlat,n_days,n_lead_times,n_times_per_day)               
               qadv_vint=fltarr(tadv_nlon,tadv_nlat,n_days,n_lead_times,n_times_per_day)
            ENDIF
            ...
            ;print,qadv_infile(0)
            qadv=OPEN_AND_EXTRACT(qadv_infile(0),'tnhusa',$
                                offset=[qadv_box_tx(1),qadv_box_tx(0),offset_p,lead_times(k)*n_times_per_day],$
                                count=[qadv_nlon,qadv_nlat,np,n_times_per_day])
            IF TOTAL(where(ABS(qadv) ge missing_value)) ge 0 THEN $
               qadv[where(ABS(qadv) ge missing_value)]=!Values.F_NaN
            qadv=qadv*qadv_multiplier
            ;print,MAX(qadv,/NaN),MAX(tadv,/NaN),N_ELEMENTS(where(FINITE(tadv) eq 0)),N_ELEMENTS(tadv)
                     ...
                     FOR n=0,tadv_nlon-1 DO BEGIN                        
                        ...
                        IF FINITE(tadv_anom) eq 1 THEN BEGIN
                           tadv_anom_vint(n,m,j,k,r)=tadv_anom_vint(n,m,j,k,r)+$
                                                     tadv_anom*(p(s)-p(s+1))
                           qadv_vint(n,m,j,k,r)=qadv_vint(n,m,j,k,r)+$
                                              qadv(n,m,s,r)*(p(s)-p(s+1))                        
                        ENDIF
                     ENDFOR
                  ENDFOR
   ...
   FOR k=0,n_qadvbins-2 DO BEGIN
      IF TOTAL(where(qadv_vint ge qadv_bins(k) and qadv_vint lt qadv_bins(k+1))) ge 0 THEN BEGIN
         pts=where(qadv_vint ge qadv_bins(k) and qadv_vint lt qadv_bins(k+1)) 
         temp_tadv=tadv_anom_vint[pts]
         mean_qadvbin(k+1)=MEAN(temp_tadv,/NaN)
         median_qadvbin(k+1)=MEDIAN(temp_tadv[where(FINITE(temp_tadv) eq 1)])
      ENDIF ELSE BEGIN
         mean_qadvbin(k+1)=!Values.F_NaN
         median_qadvbin(k+1)=!Values.F_NaN
      ENDELSE
   ENDFOR
   IF TOTAL(where(qadv_vint lt qadv_bins(0))) ge 0 THEN BEGIN
      pts=where(qadv_vint lt qadv_bins(0))
      temp_tadv=tadv_anom_vint[pts]
      mean_qadvbin(0)=MEAN(temp_tadv,/NaN)
      median_qadvbin(0)=MEDIAN(temp_tadv[where(FINITE(temp_tadv) eq 1)])
   ENDIF ELSE BEGIN
      mean_qadvbin(0)=!Values.F_NaN
      median_qadvbin(0)=!Values.F_NaN
   ENDELSE
   IF TOTAL(where(qadv_vint ge qadv_bins(n_qadvbins-1))) ge 0 THEN BEGIN
      pts=where(qadv_vint ge qadv_bins(n_qadvbins-1))
      temp_tadv=tadv_anom_vint[pts]
      mean_qadvbin(n_qadvbins)=MEAN(temp_tadv,/NaN)
      median_qadvbin(n_qadvbins)=MEDIAN(temp_tadv[where(FINITE(temp_tadv) eq 1)])
   ENDIF ELSE BEGIN
      mean_qadvbin(n_qadvbins)=!Values.F_NaN
      median_qadvbin(n_qadvbins)=!Values.F_NaN
   ENDELSE

   total_twod_bin=TOTAL(twod_bin)
   twod_bin=twod_bin/total_twod_bin

   tadv_mid=fltarr(n_tadvbins)
   tadv_mid(0)=!Values.F_NaN
   FOR j=0,n_tadvbins-2 DO $
      tadv_mid(j+1)=(tadv_bins(j)+tadv_bins(j+1))/2.
   slope_qadv=REGRESS(mean_tadvbin(1:NEAREST(tadv_bins,0)-1),tadv_bins(1:NEAREST(tadv_bins,0)-1))
   correlation_qadv=CORRELATE(mean_tadvbin(1:NEAREST(tadv_bins,0)-1),tadv_bins(1:NEAREST(tadv_bins,0)-1)*slope_qadv(0))
   qadv_mid=fltarr(n_qadvbins)
   qadv_mid(0)=!Values.F_NaN
   FOR j=0,n_qadvbins-2 DO $
      qadv_mid(j+1)=(qadv_bins(j)+qadv_bins(j+1))/2.
   slope_tadv=REGRESS(mean_qadvbin(NEAREST(qadv_bins,0):n_qadvbins-1),qadv_bins(NEAREST(qadv_bins,0):n_qadvbins-1))   
   correlation_tadv=CORRELATE(mean_qadvbin(1:NEAREST(qadv_bins,0)-1),qadv_bins(1:NEAREST(qadv_bins,0)-1)*slope_tadv(0))

   print,'Slope of qadv by Tadv for Tadv < 0 (red line): ',slope_qadv,' correlation: ',correlation_qadv
   print,'Slope of Tadv by qadv for qadv > 0 (purple line): ',slope_tadv,' correlation: ',correlation_tadv
   ...
   FOR j=0,3 DO BEGIN
      CASE j OF 
         0 : BEGIN
            x_range=[0,NEAREST(tadv_bins,0)-1]
            y_range=[0,NEAREST(qadv_bins,0)-1]
         END
         1 : BEGIN
            x_range=[NEAREST(tadv_bins,0)+1,n_tadvbins]
            y_range=[0,NEAREST(qadv_bins,0)-1]
         END
         2 : BEGIN
            x_range=[0,NEAREST(tadv_bins,0)-1]
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/q_bin/mjo_diabatic_plot_qadv_tadv_vint.'+model_desc+'_'+$
   ...
   GPLOT,X=[NEAREST(tadv_bins,0)+1.5,NEAREST(tadv_bins,0)+1.5],Y=[0,n_qadvbins+1],STYLE=1
   ...
   FOR j=0,n_qadvbins DO BEGIN
      mean_plot(j)=NEAREST(tadv_bins,mean_qadvbin(j))+1
      IF mean_plot(j) lt n_tadvbins-1 THEN $
         mean_plot(j)=mean_plot(j)+(mean_qadvbin(j)-tadv_bins(mean_plot(j)))/(tadv_bins(mean_plot(j)+1)-tadv_bins(mean_plot(j)))+1
      median_plot(j)=NEAREST(tadv_bins,median_qadvbin(j))+1
      IF median_plot(j) lt n_tadvbins-1 THEN $
         median_plot(j)=median_plot(j)+(median_qadvbin(j)-tadv_bins(median_plot(j)))/(tadv_bins(median_plot(j)+1)-tadv_bins(median_plot(j)))+1
   ENDFOR
   ...
   ENDFOR
   GPLOT,X=1,Y=2,TEXT='Slope of qadv by Tadv for Tadv < 0.0 (red): '+STRMID(STRTRIM(STRING(slope_qadv),1),0,5),ALIGN=0.0
   GPLOT,X=1,Y=3,TEXT='Slope of Tadv by qadv for qadv > 0.0 (purple): '+STRMID(STRTRIM(STRING(slope_tadv),1),0,5),ALIGN=0.0
   AXES,XVALS=indgen(n_tadvbins+2),YVALS=indgen(n_qadvbins+2),$
        ...
        ORIENTATION=30,XTITLE='Vertically integrated dTadv/dt (K day!U-1!N)',$
        YTITLE='Vertically integrated dqadv/dt (K day!U-1!N)'
