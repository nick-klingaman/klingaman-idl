PRO mjo_diabatic_ngms_20day,models,case_letter

;models=['cam5zm']
leads=indgen(20)
n_models=N_ELEMENTS(models)
n_leads=N_ELEMENTS(leads)

ngms_horiz=fltarr(n_models,n_leads)
ngms_vert=fltarr(n_models,n_leads)
...
FOR i=0,n_models-1 DO BEGIN  
   ...
   openw,lun,dir+'/ngms_'+case_letter+'.txt',/get_lun
   printf,lun,'Normalised Gross Moist Stability for Case '+case_letter

   FOR l=0,n_leads-1 DO BEGIN
      print,'   -->  At lead time '+STRTRIM(STRING(leads(l)),1)+' day(s)'
      printf,lun,'Lead time '+STRTRIM(STRING(leads(l)),1)+' day(s)'
      ...
      FOR x=0,nlon-1 DO BEGIN
         IF landsea(x,y) ne 1 THEN BEGIN
            FOR t=8,nt-9 DO BEGIN
               ...
               nt2=0            
               nrr=0
               FOR id=1,17 DO BEGIN                  
                  IF t1(x,y,t+id-9) ne -9999. THEN BEGIN
                     nt1=nt1+1
                     t1s=t1s+t1(x,y,t+id-9)
                  ENDIF
                  IF t2(x,y,t+id-9) ne -9999. THEN BEGIN
                     nt2=nt2+1
                     t2s=t2s+t2(x,y,t+id-9)
                  ENDIF                
                  IF rr(x,y,t+id-9) ne -9999. THEN BEGIN
                     nrr=nrr+1
                     rrs=rrs+rr(x,y,t+id-9)
                  ENDIF
               ENDFOR               
