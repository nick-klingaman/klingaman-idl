PRO mjo_diabatic_hovmollers_olr_withlead,model_names,start_date,stop_date,lead_times=lead_times,lead_time_nth=lead_time_nth,lon=lon_range,lat=lat_range
 
; Plot Hovmollers of observed and forecast OLR, 
; averaged over specified range of latitudes (default 10S-10N).

n_models=N_ELEMENTS(model_names)
levels=['165','175','185','195','205','215','225','235','245','255','265','275','285','295']
levels_anom=['-68','-60','-52','-44','-36','-28','-20','-12','-4','4','12','20','28','36','44','52','60','68']
...
; Get observations
obs_infile='/home/ss901165/datasets/ECMWF_YOTC/NOAA_CIRES.jun2009-feb2010_dmeans.olr.nc'
obs_clim_infile='/home/ss901165/datasets/NOAA_CIRES_OLR/daily/NOAA_CIRES_OLR.jan-dec_dmeans_clim.1989-2008.nc'
obs_start_date=152
...
obs_longitude=OPEN_AND_EXTRACT(obs_infile,'lon')
obs_latitude=OPEN_AND_EXTRACT(obs_infile,'lat')
...
ENDIF ELSE IF stop_julian gt 365 and start_julian gt 365 THEN BEGIN
   obs_climatology=OPEN_AND_EXTRACT(obs_clim_infile,'olr',$
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/olr/plots/mjo_diabatic_hovmollers_olr_withlead.obs_'+$
...
PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
       CB_WIDTH=110,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='Daily-mean OLR from NOAA CIRES obs for '+$
     start_date+'-'+stop_date
CS,SCALE=1,NCOLS=N_ELEMENTS(levels)+1
LEVS,MANUAL=levels
CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=obs_olr_latavg,CB_TITLE='Outgoing longwave radiation (W m!U-2!N)',$
    /NOLINES
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/olr/plots/mjo_diabatic_hovmollers_olr_withlead.obs_anom-mean-period_'+$
       start_date+'-'+stop_date+'.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
       CB_WIDTH=110,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='NOAA CIRES OLR anom from period-mean for '+$
     start_date+'-'+stop_date
CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
LEVS,MANUAL=levels_anom
toplot=fltarr(obs_nlon,n_days)
FOR i=0,obs_nlon-1 DO $
   toplot(i,*)=obs_olr_latavg(i,*)-MEAN(obs_olr_latavg(i,*),/NaN)
CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,CB_TITLE='Outgoing longwave radiation (W m!U-2!N)',$
    /NOLINES
ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                            where(dates eq start_date)+n_days)),1)
AXES,XSTEP=(our_stop_lon-our_start_lon)/10,YVALS=indgen(n_days/2)*2,YLABELS=ylabels(0:N_ELEMENTS(ylabels)-1:2),NDECS=2,$
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/olr/plots/mjo_diabatic_hovmollers_olr_withlead.obs_anom-clim_'+$
       start_date+'-'+stop_date+'.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
       CB_WIDTH=110,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='NOAA CIRES OLR anom from daily clim (89-08) for '+$
     start_date+'-'+stop_date
CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
LEVS,MANUAL=levels_anom
toplot=obs_olr_latavg-obs_climatology_latavg
CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,CB_TITLE='Outgoing longwave radiation (W m!U-2!N)',$
    /NOLINES
ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                            where(dates eq start_date)+n_days)),1)
...
FOR i=0,n_models-1 DO BEGIN
   ...
   this_model_olr_latavg=fltarr(obs_nlon,n_lead_times,n_days)
   grid_flag=0
...
;                  ' at lead time '+STRTRIM(STRING(our_lead_times(k)),1)
            this_model_olr_latavg(*,k,j)=!Values.F_NaN
         ENDIF ELSE BEGIN
...
;                  ' using hindcast initialised on '+initial_date
            model_infile=indir+'/'+REFORM(initial_date)+'/'+file_desc+'.rlut.'+REFORM(initial_date)+'.00Z.nc'
            IF grid_flag eq 0 THEN BEGIN
               ...
               grid_flag=1
            ENDIF
            today_olr=OPEN_AND_EXTRACT(model_infile(0),'rlut',$
                                       offset=[model_box_tx(1),model_box_tx(0),our_lead_times(k)*model_times_per_day],$
                                       count=[model_nlon,model_nlat,model_times_per_day])
            IF TOTAL(where(ABS(today_olr) ge 1e5)) ge 0 THEN $
               today_olr[where(ABS(today_olr) ge 1e5)]=!Values.F_NaN
            IF TOTAL(where(ABS(today_olr) le 0.001)) ge 0 THEN $
               today_olr[where(ABS(today_olr) le 0.001)]=!Values.F_NaN
            FOR m=0,model_nlon-1 DO $
               this_model_olr_latavg(m,k,j)=MEAN(today_olr(m,*,*),/NaN)
         ENDELSE
      ENDFOR
   ENDFOR

   IF has_climatology eq 1 THEN BEGIN
      clim_infile=clim_indir+'/'+file_desc+'.jan-dec_dmean_clim.1991-2010.rlut.nc'
   ...
   FOR j=0,n_lead_times-1 DO BEGIN
      psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/olr/plots/mjo_diabatic_hovmollers_olr_withlead.'+plot_desc+'_lead'+$
             STRTRIM(STRING(our_lead_times(j)),1)+'_'+start_date+'-'+stop_date+'.ps'
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=110,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
      GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='Daily-mean OLR from '+plot_desc+' at lead '+$
           STRTRIM(STRING(our_lead_times(j)),1)+' for '+start_date+'-'+stop_date
      CS,SCALE=1,NCOLS=N_ELEMENTS(levels)+1
      LEVS,MANUAL=levels
      toplot=REFORM(this_model_olr_latavg(*,j,*))
      ...
      CON,X=model_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,$
          CB_TITLE='Outgoing longwave radiation (W m!U-2!N)',/NOLINES
      ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                   where(dates eq start_date)+n_days)),1)
      AXES,XSTEP=(our_stop_lon-our_start_lon)/10,YVALS=indgen(n_days/2)*2,YLABELS=ylabels(0:N_ELEMENTS(ylabels)-1:2),NDECS=2,$
           YTITLE='Date',XTITLE='Longitude (degrees east)',/NORIGHT
      ...
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=110,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
      GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='OLR anom from period mean from '+plot_desc+' at lead '+$
           STRTRIM(STRING(our_lead_times(j)),1)+' for '+start_date+'-'+stop_date
      CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
      LEVS,MANUAL=levels_anom
      toplot=REFORM(this_model_olr_latavg(*,j,*))
      ...
      IF has_climatology eq 1 THEN BEGIN
         ...
         PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
                CB_WIDTH=110,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
         GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='OLR anom from daily clim (20yr) from '+plot_desc+' at lead '+$
              STRTRIM(STRING(our_lead_times(j)),1)+' for '+start_date+'-'+stop_date
         CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
         LEVS,MANUAL=levels_anom
         toplot=REFORM(this_model_olr_latavg(*,j,*))-REFORM(climatology_latavg)
         IF TOTAL(where(toplot eq 0)) ge 0 THEN $
            toplot[where(toplot eq 0)]=!Values.F_NaN
         FOR k=0,model_nlon-1 DO $
            toplot(k,*)=toplot(k,*)-MEAN(toplot(k,*),/NaN)    
