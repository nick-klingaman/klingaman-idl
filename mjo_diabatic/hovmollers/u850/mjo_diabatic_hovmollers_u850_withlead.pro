PRO mjo_diabatic_hovmollers_u850_withlead,model_names,start_date,stop_date,lead_times=lead_times,lead_time_nth=lead_time_nth,lon=lon_range,lat=lat_range
 
; Plot Hovmollers of observed and forecast U850, 
; averaged over specified range of latitudes (default 10S-10N).

n_models=N_ELEMENTS(model_names)
levels=['-10.2','-9.0','-7.8','-6.6','-5.4','-4.2','-3.0','-1.8','-0.6',$
        '0.6','1.8','3.0','4.2','5.4','6.6','7.8','9.0','10.2']
levels_anom=['-8.5','-7.5','-6.5','-5.5','-4.5','-3.5','-2.5','-1.5','-0.5',$
...
; Get observations
obs_infile='/home/ss901165/datasets/ECMWF_YOTC/ECMWF_YOTC.oct2009-feb2010_dmeans.u850.nc'
obs_clim_infile='/home/ss901165/datasets/ERA-INTERIM/U850/U850.jan-dec_dmean_clim.1989-2009.mjo_domain.2.5x2.5.nc'
obs_start_date=274
...
obs_u850=OPEN_AND_EXTRACT(obs_infile,'U',$
...
obs_u850_latavg=fltarr(obs_nlon,n_days)
FOR i=0,obs_nlon-1 DO $
   FOR j=0,n_days-1 DO $
      obs_u850_latavg(i,j)=MEAN(obs_u850(i,*,j*obs_times_per_day:(j+1)*obs_times_per_day-1))
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_withlead.obs_'+$
...
GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='Daily-mean U850 from ERA-Int for '+$
     start_date+'-'+stop_date
CS,SCALE=1,NCOLS=N_ELEMENTS(levels)+1
LEVS,MANUAL=levels
CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=obs_u850_latavg,CB_TITLE='850 hPa zonal wind (m s!U-1!N)',$
    /NOLINES
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_withlead.obs_anom-mean-period_'+$
       start_date+'-'+stop_date+'.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
       CB_WIDTH=120,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='ERA-Int U850 anom from period-mean for '+$
     start_date+'-'+stop_date
CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
LEVS,MANUAL=levels_anom
toplot=fltarr(obs_nlon,n_days)
FOR i=0,obs_nlon-1 DO $
   toplot(i,*)=obs_u850_latavg(i,*)-MEAN(obs_u850_latavg(i,*),/NaN)
CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,CB_TITLE='850 hPa zonal wind (m s!U-1!N)',$
    /NOLINES
ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                            where(dates eq start_date)+n_days)),1)
AXES,XSTEP=(our_stop_lon-our_start_lon)/10,YVALS=indgen(n_days/2)*2,YLABELS=ylabels(0:N_ELEMENTS(ylabels)-1:2),NDECS=2,$
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_withlead.obs_anom-clim_'+$
       start_date+'-'+stop_date+'.ps'
PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
       CB_WIDTH=120,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='ERA-Int U850 anom from daily clim (89-08) for '+$
     start_date+'-'+stop_date
CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
LEVS,MANUAL=levels_anom
toplot=obs_u850_latavg-obs_climatology_latavg
CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,CB_TITLE='850 hPa zonal wind (m s!U-1!N)',$
    /NOLINES
ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                            where(dates eq start_date)+n_days)),1)
...
FOR i=0,n_models-1 DO BEGIN
   ...
   this_model_u850_latavg=fltarr(obs_nlon,n_lead_times,n_days)
   grid_flag=0
...
;                  ' at lead time '+STRTRIM(STRING(our_lead_times(k)),1)
            this_model_u850_latavg(*,k,j)=!Values.F_NaN
         ENDIF ELSE BEGIN
...
;                  ' using hindcast initialised on '+initial_date
            model_infile=indir+'/'+REFORM(initial_date)+'/'+file_desc+'.ua_850hPa.'+REFORM(initial_date)+'.00Z.nc'
            IF grid_flag eq 0 THEN BEGIN
               ...
               grid_flag=1
            ENDIF
            today_u850=OPEN_AND_EXTRACT(model_infile(0),'ua',$
                                       offset=[model_box_tx(1),model_box_tx(0),our_lead_times(k)*model_times_per_day],$
                                       count=[model_nlon,model_nlat,model_times_per_day])
            IF TOTAL(where(ABS(today_u850) ge 1e5)) ge 0 THEN $
               today_u850[where(ABS(today_u850 ge 1e5))]=!Values.F_NaN            
            IF TOTAL(where(ABS(today_u850) le 0.001)) ge 0 THEN $
               today_u850[where(ABS(today_u850) le 0.001)]=!Values.F_NaN
            FOR m=0,model_nlon-1 DO $
               this_model_u850_latavg(m,k,j)=MEAN(today_u850(m,*,*),/NaN)
         ENDELSE
      ENDFOR
   ENDFOR

   IF has_climatology eq 1 THEN BEGIN
      clim_infile=clim_indir+'/'+file_desc+'.jan-dec_dmean_clim.1991-2010.ua_850hPa.nc'
   ...
   FOR j=0,n_lead_times-1 DO BEGIN
      psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_withlead.'+plot_desc+'_lead'+$
             STRTRIM(STRING(our_lead_times(j)),1)+'_'+start_date+'-'+stop_date+'.ps'
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=120,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
      GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='Daily-mean U850 from '+plot_desc+' at lead '+$
           STRTRIM(STRING(our_lead_times(j)),1)+' for '+start_date+'-'+stop_date
      CS,SCALE=1,NCOLS=N_ELEMENTS(levels)+1
      LEVS,MANUAL=levels
      toplot=REFORM(this_model_u850_latavg(*,j,*))
      ...
      CON,X=model_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,$
          CB_TITLE='850 hPa zonal wind (m s!U-1!N)',/NOLINES
      ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                   where(dates eq start_date)+n_days)),1)
      AXES,XSTEP=(our_stop_lon-our_start_lon)/10,YVALS=indgen(n_days/2)*2,YLABELS=ylabels(0:N_ELEMENTS(ylabels)-1:2),NDECS=2,$
           YTITLE='Date',XTITLE='Longitude (degrees east)',/NORIGHT
      ...
      PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
             CB_WIDTH=120,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
      GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='U850 anom from period mean from '+plot_desc+' at lead '+$
           STRTRIM(STRING(our_lead_times(j)),1)+' for '+start_date+'-'+stop_date
      CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
      LEVS,MANUAL=levels_anom
      toplot=REFORM(this_model_u850_latavg(*,j,*))
      ...
      IF has_climatology eq 1 THEN BEGIN
         ...
         PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
                CB_WIDTH=120,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
         GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='U850 anom from daily clim (20yr) from '+plot_desc+' at lead '+$
              STRTRIM(STRING(our_lead_times(j)),1)+' for '+start_date+'-'+stop_date
         CS,SCALE=1,NCOLS=N_ELEMENTS(levels_anom)+1
         LEVS,MANUAL=levels_anom
         toplot=REFORM(this_model_u850_latavg(*,j,*))-REFORM(climatology_latavg)
         IF TOTAL(where(toplot eq 0)) ge 0 THEN $
            toplot[where(toplot eq 0)]=!Values.F_NaN
         FOR k=0,model_nlon-1 DO $
            toplot(k,*)=toplot(k,*)-MEAN(toplot(k,*),/NaN)    
