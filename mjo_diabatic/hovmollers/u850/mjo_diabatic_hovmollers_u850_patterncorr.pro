PRO mjo_diabatic_hovmollers_u850_patterncorr,model_names,start_date1,stop_date1,start_date2,stop_date2,lead_times=lead_times,lon=lon_range,lat=lat_range
...
FOR d=0,1 DO BEGIN
   ...
   IF stop_julian lt start_julian THEN $
      stop_julian=stop_julian+365
                                ...
                                ; Get observations
   obs_infile='/home/ss901165/datasets/ECMWF_YOTC/ECMWF_YOTC.jun2009-feb2010_dmeans.u850.2.5x2.5.nc'
   ...
   IF TOTAL(where(obs_u850 ge 1e10)) ge 0 THEN $
      obs_u850[where(obs_u850 ge 1e10)]=!Values.F_NaN
   
   obs_u850_latavg=fltarr(obs_nlon,n_days)
   FOR i=0,obs_nlon-1 DO $
      FOR j=0,n_days-1 DO $
         obs_u850_latavg(i,j)=MEAN(obs_u850(i,*,j),/NaN)   
   FOR j=0,n_days-1 DO $
      obs_u850_latavg(*,j)=SMOOTH(obs_u850_latavg(*,j),5)
   
   psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_patterncorr.obs_'+$
   ...
   GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days;,TITLE='Daily-mean u850 from TRMM 3B42v6A obs for '+$
        ;start_date+'-'+stop_date
   CS,SCALE=1,NCOLS=N_ELEMENTS(levels)+1,/REV
   LEVS,MANUAL=levels
   CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=obs_u850_latavg,CB_TITLE='U850itation (mm day!U-1!N)',$
       /NOLINES
...
;!guide.thick=1.0
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_patterncorr.obs_anom-mean-period_'+$
          start_date+'-'+stop_date+'.ps'
   PSOPEN,file=psfile,FONT=2,CHARSIZE=120,MARGIN=2500,SPACE3=200,XOFFSET=1500,YOFFSET=2000,TFONT=2,TCHARSIZE=90,$
          CB_WIDTH=120,XSIZE=15000,YSIZE=23000,SPACE2=2000,/PORTRAIT
   GSET,XMIN=MIN(obs_longitude),XMAX=MAX(obs_longitude),YMIN=0,YMAX=n_days,TITLE='TRMM u850 anom from period-mean for '+$
        start_date+'-'+stop_date
   ...
   CON,X=obs_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,CB_TITLE='U850itation (mm day!U-1!N)',$
       /NOLINES
   ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                where(dates eq start_date)+n_days)),1)
   AXES,XSTEP=(our_stop_lon-our_start_lon)/10,YVALS=indgen(n_days/2)*2,YLABELS=ylabels(0:N_ELEMENTS(ylabels)-1:2),NDECS=2,$
...
;                  ' using hindcast initialised on '+initial_date
            ...
            ENDIF
               ...
               IF TOTAL(where(ABS(today_u850) ge 1e10)) ge 0 THEN $
                  today_u850[where(ABS(today_u850) ge 1e10)]=!Values.F_NaN
               FOR m=0,model_nlon-1 DO $
                  this_model_u850_latavg(m,k,j)=MEAN(today_u850(m,*,*),/NaN)
               this_model_u850_latavg(*,k,j)=SMOOTH(this_model_u850_latavg(*,k,j),5)
                                ;IF k eq 0 THEN  $
                                ;print,initial_date,MEAN(today_u850,/NaN)
            ENDELSE
         ENDFOR      
      ENDFOR
      
      FOR j=0,n_lead_times-1 DO BEGIN
         psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/plots/mjo_diabatic_hovmollers_u850_patterncorr.'+plot_desc+'_lead'+$
         ...
         CON,X=model_longitude,Y=indgen(n_days)+0.5,FIELD=toplot,$
             CB_TITLE='U850itation (mm day!U-1!N)',/NOLINES
         ylabels=STRTRIM(STRING(dates(where(dates eq start_date):$
                                      where(dates eq start_date)+n_days)),1)
         AXES,XSTEP=(our_stop_lon-our_start_lon)/10,YVALS=indgen(n_days/2)*2,YLABELS=ylabels(0:N_ELEMENTS(ylabels)-1:2),NDECS=2,$
              YTITLE='Date',XTITLE='Longitude (degrees east)'
...
;      !guide.thick=1.0
         ...
         pcorr(i,j,d)=CORRELATE(REFORM(obs_u850_latavg,[obs_nlon*n_days]),REFORM(this_model_u850_latavg(*,j,*),[model_nlon*n_days]))
         IF j gt 0 THEN $
            pcorr(i,j,d)=pcorr(i,j,d)
...
psfile='/home/ss901165/idl/mjo_diabatic/hovmollers/u850/mjo_diabatic_hovmollers_u850_patterncorr.common_models_pcorr.ps'
PSOPEN,file=psfile,FONT=6,TFONT=6,CHARSIZE=160,MARGIN=2000,YOFFSET=1000,XOFFSET=4500,XSIZE=16000,YSIZE=16000
GSET,XMIN=0,XMAX=23,YMIN=-0.05,YMAX=1.0
overlap=[999]
