PRO mjo_diabatic_precip_bin_qanom_twod_withlead,model_names=model_names,start_date,stop_date,lead_times,box,box_name,mask_type,plot_yotc=plot_yotc
...
mylevs=['-2.4','-1.9','-1.5','-1.2','-0.9','-0.6','-0.4','-0.3','-0.2','-0.1','-0.05',$
        '0.05','0.1','0.2','0.3','0.4','0.6','0.9','1.2','1.5','1.9','2.4']
...
precip_bins=[0.1,0.3,0.6,1.0,1.5,2.0,3.0,4.0,5.0,7.0,9.0,12.0,16.0,20.0,30.0]
...
IF KEYWORD_SET(plot_yotc) THEN BEGIN
   ...
   ecmwf_infile='/home/ss901165/datasets/ECMWF_YOTC/ECMWF_YOTC.oct2009-feb2010_3hrmeans_0-24hfcst.q.nc'
   ...
   ecmwf_q=OPEN_AND_EXTRACT(ecmwf_infile,'Q',offset=[ecmwf_box_tx(1),ecmwf_box_tx(0),0,ecmwf_start_offset],$
                            count=[ecmwf_nlon,ecmwf_nlat,ecmwf_nz,ecmwf_count])*1000.0
...
;   trmm_infile='/home/ss901165/datasets/TRMM_3B42V6/two_point_five/TRMM_3B42v6A.oct2009-feb2010_3hrmeans.precip.2.5x2.5.nc'
   ...
   FOR m=0,ecmwf_nz-1 DO BEGIN
      ...
      FOR n=0,n_bins-1 DO BEGIN
         ...
         IF TOTAL(valid_points) ne -1 THEN BEGIN
            q_binned(n+1,m)=q_binned(n+1,m)+TOTAL(q_thislev[valid_points],/NaN)
            IF TOTAL(where(q_thislev[valid_points] gt 0)) ge 0 THEN $
               q_pctwet(n+1,m)=q_pctwet(n+1,m)+N_ELEMENTS(where(q_thislev[valid_points] gt 0))
            IF m eq 0 THEN $
               n_pts(n+1)=n_pts(n+1)+N_ELEMENTS(valid_points)
         ENDIF
      ENDFOR              
      valid_points=where(trmm_precip lt precip_bins(0) and FINITE(trmm_precip) eq 1)
      q_binned(0,m)=q_binned(0,m)+TOTAL(q_thislev[valid_points],/NaN)
      q_pctwet(0,m)=q_pctwet(0,m)+N_ELEMENTS(where(q_thislev[valid_points] gt 0))
      IF m eq 0 THEN $
         n_pts(0)=n_pts(0)+N_ELEMENTS(valid_points)
   ENDFOR

   FOR n=0,n_bins DO BEGIN
      q_binned(n,*)=q_binned(n,*)/FLOAT(n_pts(n))
      q_pctwet(n,*)=q_pctwet(n,*)/FLOAT(n_pts(n))*100.
   ENDFOR

   outfile='/home/ss901165/um_output6/mjodiab_20day/ECMWF_YOTC_TRMM_qanom_byprecip_'+box_name+'.nc'
   ...
   NCDF_VARPUT,id,varids(2),q_binned
   NCDF_VARPUT,id,varids(3),q_pctwet
   NCDF_CLOSE,id

   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_qanom_twod_withlead.ecmwf_yotc_trmm_'+$
          STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+'_'+box_name+'.ps'
   PSOPEN,file=psfile,FONT=6,CHARSIZE=160,MARGIN=3500,SPACE1=250,SPACE2=500,XOFFSET=0,YOFFSET=0,TFONT=6,$
          TCHARSIZE=90,SPACE3=600,CB_WIDTH=110
   GSET,XMIN=0,XMAX=n_bins+1,YMIN=MAX(ecmwf_z),YMAX=MIN(ecmwf_z)
   CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs)+1,/REV,white=[13]
   LEVS,MANUAL=mylevs
   CON,Y=ecmwf_z,X=indgen(n_bins+1)+0.5,FIELD=q_binned,/BLOCK,/NOLINES,$
       CB_TITLE='Specific humidity (g kg!U-1!N) anomaly from zonal mean ',/NOCOLBAR ;,$
                                ;TITLE='q anomalies binned by precip over '+box_name+' '+mask_type+' - ECMWF_YOTC q and precip - dates '+$
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_qanom_twod_withlead.ecmwf_yotc_trmm_'+$
          STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+'_'+box_name+'_pctwet.ps'
   PSOPEN,file=psfile,FONT=3,CHARSIZE=100,MARGIN=2500,SPACE1=100,SPACE2=500,XOFFSET=0,YOFFSET=0,TFONT=3,$
          TCHARSIZE=90,SPACE3=600,CB_WIDTH=110
   GSET,XMIN=0,XMAX=n_bins+1,YMIN=MAX(ecmwf_z),YMAX=MIN(ecmwf_z)
   CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs_pct)+1,/REV
   LEVS,MANUAL=mylevs_pct
   CON,Y=ecmwf_z,X=indgen(n_bins+1)+0.5,FIELD=q_pctwet,/BLOCK,/NOLINES,$
       CB_TITLE='Percentage of points in bin with wet anom (%)',$
       TITLE='Percent of points with wet anom in '+box_name+' '+mask_type+' - ECMWF_YOTC q and precip - dates '+$
       STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)  
...
FOR i=0,n_models-1 DO BEGIN
   ...
   FOR j=0,n_days(0)-1 DO BEGIN
      ...
      IF TOTAL(where(valid_dates eq standard_valid_dates(j+start_position(0)))) ne -1 THEN BEGIN     
         FOR k=0,n_lead_times-1 DO BEGIN    
            ...
            ;print,initial_date
            model_q_infile=indir+'/'+REFORM(initial_date)+'/'+file_desc+'.hus.'+REFORM(initial_date)+'.00Z.nc'
            ...
            IF grid_flag eq 0 THEN BEGIN
               ...
               model_z=OPEN_AND_EXTRACT(model_q_infile(0),level_name)*level_multiplier
               ...
               model_q_longitude=OPEN_AND_EXTRACT(model_q_infile(0),longitude_name)
               model_q_latitude=OPEN_AND_EXTRACT(model_q_infile(0),latitude_name)
               DEFINE_BOUNDARIES,box,model_q_latitude,model_q_longitude,model_q_box_tx,/LIMIT
               ...
               model_q=fltarr(model_nlon,model_nlat,model_nz,n_times_per_day)    
               q_binned=fltarr(n_bins+1,model_nz)
               q_pctwet=fltarr(n_bins+1,model_nz)
               n_pts=fltarr(n_bins+1)
            ENDIF
            model_precip(*,*,*)=$            
            ...
            ENDFOR
            model_q(*,*,*,*)=$
               OPEN_AND_EXTRACT(model_q_infile(0),'hus',$
                                offset=[model_q_box_tx(1),model_q_box_tx(0),0,lead_times(k)*n_times_per_day],$
                                count=[model_nlon,model_nlat,model_nz,n_times_per_day])*q_multiplier
            IF TOTAL(where(ABS(model_q) ge missing_value)) gt 0 THEN $
               model_q[where(ABS(model_q) ge missing_value)]=!Values.F_NaN
            
            FOR m=0,model_nlat-1 DO BEGIN
               FOR n=0,model_nz-1 DO BEGIN
                  model_q_lonavg=MEAN(model_q(*,m,n,*),/NaN)   
                  model_q(*,m,n,*)=model_q(*,m,n,*)-model_q_lonavg
               ENDFOR
            ENDFOR
            
            FOR m=0,model_nz-1 DO BEGIN
               q_thislev=REFORM(model_q(*,*,m,*))
               FOR n=0,n_bins-1 DO BEGIN
                  IF n ne n_bins-1 THEN BEGIN
   ...
   outfile=indir+'/'+file_desc+'_qanom_byprecip_'+box_name+'.nc'
   id=NCDF_CREATE(outfile,/CLOBBER)
   dimids=intarr(2)
   dimids(0)=NCDF_DIMDEF(id,'precip',n_bins+1)
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_qanom_twod_withlead.'+model_desc+'_'+$
   ...
   PSOPEN,file=psfile,FONT=6,CHARSIZE=140,MARGIN=3500,SPACE1=250,SPACE2=500,XOFFSET=0,YOFFSET=0,TFONT=6,$
          TCHARSIZE=100,SPACE3=600,CB_WIDTH=110
   GSET,XMIN=0,XMAX=n_bins+1,YMIN=MAX(model_z),YMAX=MIN(model_z)
   CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs)+1,/REV,white=[13]
   LEVS,MANUAL=mylevs
   CON,Y=model_z,X=indgen(n_bins+1)+0.5,FIELD=q_binned,/BLOCK,/NOLINES,CB_WIDTH=130,$
       CB_TITLE='Specific humidity (g kg!U-1!N) anomaly from zonal mean ',/NOCOLBAR ;,$
                                ;TITLE='q anomalies binned by precip over '+box_name+' '+mask_type+' - '+model_desc+' dates '+$
   ...
   AXES,YVALS=FLOOR(model_z[where(model_z ne 925 and model_z ne 975 and (FLOOR(model_z+0.01) MOD 100) eq 0)]),$
        XVALS=indgen(n_bins+2),XLABELS=['< '+STRMID(STRTRIM(STRING(precip_bins(0)),1),0,4),$
                                        STRMID(STRTRIM(STRING(precip_bins),1),0,4),'> '+$
                                        STRMID(STRTRIM(STRING(precip_bins(n_bins-1)),1),0,4)],$
        ORIENTATION=20,/NORIGHT,YTITLE='Pressure (hPa)',XTITLE='Daily-mean precipitation rate (mm day!U-1!N)',NDECS=2      
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_qanom_twod_withlead.'+model_desc+'_'+$
          STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+'_leads'+$
          STRTRIM(STRING(MIN(lead_times)),1)+'to'+STRTRIM(STRING(MAX(lead_times)),1)+'_'+box_name+'_pctwet.ps'
   PSOPEN,file=psfile,FONT=3,CHARSIZE=100,MARGIN=2500,SPACE1=100,SPACE2=500,XOFFSET=0,YOFFSET=0,TFONT=3,$
          TCHARSIZE=100,SPACE3=600,CB_WIDTH=110
   GSET,XMIN=0,XMAX=n_bins+1,YMIN=MAX(model_z),YMAX=MIN(model_z)
   CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs_pct)+1,/REV
   LEVS,MANUAL=mylevs_pct
   CON,Y=model_z,X=indgen(n_bins+1)+0.5,FIELD=q_pctwet,/BLOCK,/NOLINES,$
       CB_TITLE='Percentage of points in bin with wet anom (%)',$
       TITLE='Percent of points with wet anom in '+box_name+' '+mask_type+' - '+model_desc+' dates '+$
       STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+' leads '+$
       STRTRIM(STRING(MIN(lead_times)),1)+' to '+STRTRIM(STRING(MAX(lead_times)),1)
