PRO mjo_diabatic_precip_bin_dTdt_twod_withlead,model_names=model_names,start_date,stop_date,lead_times,box,box_name,mask_type,plot_yotc=plot_yotc
...
mylevs=['-2.6','-2.2','-1.8','-1.4','-1.0','-0.6','-0.2','0.2','0.6','1.0','1.4','1.8','2.2','2.6']
...
IF KEYWORD_SET(plot_yotc) THEN BEGIN
   ...
   ecmwf_infile='/home/ss901165/datasets/ECMWF_YOTC/ECMWF_YOTC.oct2009-feb2010_3hrmeans.tntr.nc'
   ...
   ecmwf_dTdt=OPEN_AND_EXTRACT(ecmwf_infile,'tntr',offset=[ecmwf_box_tx(1),ecmwf_box_tx(0),0,ecmwf_start_offset],$
...
;   trmm_infile='/home/ss901165/datasets/TRMM_3B42V6/two_point_five/TRMM_3B42v6A.oct2009-feb2010_3hrmeans.precip.2.5x2.5.nc'
   ...
   FOR m=0,n_times_per_day-1 DO BEGIN
      ...
      trmm_precip(*,*,m)=temp
   ENDFOR   
   FOR m=0,ecmwf_nlat-1 DO $
      FOR n=0,ecmwf_nz-1 DO $
         FOR p=0,ecmwf_count-1 DO $
            ecmwf_dTdt_lonavg=MEAN(ecmwf_dTdt(*,m,n,p),/NaN)   
;            ecmwf_dTdt(*,m,n,p)=ecmwf_dTdt(*,m,n,p)-ecmwf_dTdt_lonavg
   ...
   FOR m=0,ecmwf_nz-1 DO BEGIN
      ...
      FOR n=0,n_bins-1 DO BEGIN
         ...
         IF TOTAL(valid_points) ne -1 THEN BEGIN
            ...
            IF TOTAL(where(dTdt_thislev[valid_points] lt 0)) ge 0 THEN $
      ;         dTdt_pctascent(n+1,m)=dTdt_pctascent(n+1,m)+N_ELEMENTS(where(dTdt_thislev[valid_points] lt 0))
            IF m eq 0 THEN $
...
;      dTdt_pctascent(0,m)=dTdt_pctascent(0,m)+N_ELEMENTS(where(dTdt_thislev[valid_points] lt 0))
      IF m eq 0 THEN $
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_dTdtrad_twod_withlead.ecmwf_yotc_trmm_'+$
   ...
   CS,SCALE=1,NCOLS=N_ELEMENTS(mylevs)+1,white=9
   LEVS,MANUAL=mylevs
   CON,Y=ecmwf_z,X=indgen(n_bins+1)+0.5,FIELD=dTdt_binned,/BLOCK,/NOLINES,$
       CB_TITLE='dT/dt from radiation (Qr; K day!U-1!N)',$
       TITLE='Qr binned by precip over '+box_name+' - ECMWF_YOTC Q and precip - dates '+$
...
;   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_dTdtrad_pcttotal_twod_withlead.ecmwf_yotc_trmm_'+$
;          STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+'_'+box_name+'.ps'
...
;       CB_TITLE='Fraction of total vertically integrated dT/dt from radiation (Qr; K day!U-1!N) above this level',$
;       TITLE='Qr binned by precip over '+box_name+' - ECMWF_YOTC Q and precip - dates '+$
;       STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)   
;   AXES,YVALS=ecmwf_z,XVALS=indgen(n_bins+2),XLABELS=['< '+STRMID(STRTRIM(STRING(precip_bins(0)),1),0,4),$
;                                                      STRMID(STRTRIM(STRING(precip_bins),1),0,4),'> '+$
...
;         dTdt_multiplier=86400.
   ...
   FOR j=0,n_days(0)-1 DO BEGIN
      ...
      IF TOTAL(where(valid_dates eq standard_valid_dates(j+start_position(0)))) ne -1 THEN BEGIN     
         FOR k=0,n_lead_times-1 DO BEGIN    
            ...
            ;print,initial_date
            model_dTdt_infile=indir+'/'+REFORM(initial_date)+'/'+file_desc+'.tnt_radiation_clearsky.'+REFORM(initial_date)+'.00Z.nc'
            ...
            ENDFOR
            model_dTdt(*,*,*,*)=$
               OPEN_AND_EXTRACT(model_dTdt_infile(0),'tnt_radiation_clearsky',$
            ...
            FOR m=0,model_nlat-1 DO $
               FOR n=0,model_nz-1 DO $
                  model_dTdt_lonavg=MEAN(model_dTdt(*,m,n,*),/NaN)   
   ...
   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_dTdtradcs_twod_withlead.'+model_desc+'_'+$
   ...
   CON,Y=model_z,X=indgen(n_bins+1)+0.5,FIELD=dTdt_binned,/BLOCK,/NOLINES,$
       CB_TITLE='dT/dt from clearsky radiation (Qrcs) (K day!U-1!N)',$
       TITLE='Qrcs binned by precip over '+box_name+' - '+model_desc+' dates '+$
...
;   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_dTdtphys_pcttotal_twod_withlead.'+model_desc+'_'+$
;          STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+'_leads'+$
;          STRTRIM(STRING(MIN(lead_times)),1)+'to'+STRTRIM(STRING(MAX(lead_times)),1)+'_'+box_name+'.ps'
;   PSOPEN,file=psfile,FONT=3,CHARSIZE=100,MARGIN=2500,SPACE1=150,SPACE2=500,XOFFSET=0,YOFFSET=0,TFONT=3,$
;          TCHARSIZE=100,SPACE3=600,CB_WIDTH=110
...
;       TITLE='Q1-Qr binned by precip over '+box_name+' - '+model_desc+' dates '+$
;       STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+' leads '+$
;       STRTRIM(STRING(MIN(lead_times)),1)+' to '+STRTRIM(STRING(MAX(lead_times)),1)
;   
;   AXES,YVALS=model_z,XVALS=indgen(n_bins+2),XLABELS=['< '+STRMID(STRTRIM(STRING(precip_bins(0)),1),0,4),$
...
;   PSCLOSE,/NOVIEW
;
;   psfile='/home/ss901165/idl/mjo_diabatic/precip_bin/mjo_diabatic_precip_bin_dTdtphys_pcttotal_twod_withlead.'+model_desc+'-mi;nus-yotc_'+$
;          STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+'_leads'+$
;          STRTRIM(STRING(MIN(lead_times)),1)+'to'+STRTRIM(STRING(MAX(lead_times)),1)+'_'+box_name+'.ps'
;   PSOPEN,file=psfile,FONT=3,CHARSIZE=100,MARGIN=2500,SPACE1=150,SPACE2=500,XOFFSET=0,YOFFSET=0,TFONT=3,$
;          TCHARSIZE=100,SPACE3=600,CB_WIDTH=110
...
;       TITLE='Diff in frac of vint Q1-Qr by precip over '+box_name+' - '+model_desc+' minus YOTC - dates '+$
;       STRTRIM(STRING(start_date),1)+'-'+STRTRIM(STRING(stop_date),1)+' leads '+$
;       STRTRIM(STRING(MIN(lead_times)),1)+' to '+STRTRIM(STRING(MAX(lead_times)),1)
;   
;   AXES,YVALS=ecmwf_z,XVALS=indgen(n_bins+2),XLABELS=['< '+STRMID(STRTRIM(STRING(precip_bins(0)),1),0,4),$
