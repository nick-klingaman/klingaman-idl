...
input_directory='/export/mango/data-10/ss901165/20THC_REANALYSIS/every_member/u850'
field_name='u850'
netcdf_variable_name='u850'
...
bins=[-18,-14,-10,-6,-2,2,6,10,14,18]
...
FOR i=0,n_sets-1 DO BEGIN
   CASE i OF
      0 : BEGIN
         ...
         remove_clim=0          ; Do you want to remove the daily climatology, to create anomalies for each year? (1=yes, 0=no)
         remove_years=1         ; Do you want to remove the mean of some other years?  (1=yes, 0=no; if yes, list years below)
         years_to_remove=[1920,1923,1946,1955,1962,1974,1977,1982]
      END
      1 : BEGIN
   ...
   psfile='/home/ss901165/idl/mjo_enso/mjo_enso_plot_20cr_boxavg_timeseries.'+netcdf_variable_name+$
   ...
   GSET,XMIN=date_start,XMAX=date_stop+1,YMIN=-20,YMAX=20
   FOR j=0,n_years-1 DO BEGIN
      ...
      ENDFOR
   ENDFOR
   AXES,XSTEP=3,YSTEP=2,YTITLE=netcdf_variable_name+' area averaged over lat '+STRTRIM(STRING(box(0)),1)+' to '+$
   ...
   psfile='/home/ss901165/idl/mjo_enso/mjo_enso_plot_20cr_boxavg_timeseries.'+netcdf_variable_name+$
          ...
          STRTRIM(STRING(date_stop),1)+'.ps'
   PSOPEN,file=psfile,CHARSIZE=120
   GSET,XMIN=0,XMAX=n_bins,YMIN=0,YMAX=0.8
   FOR j=0,n_years-1 DO BEGIN
      pdf=fltarr(n_bins)
      ...
      print,'Skewness of year '+STRTRIM(STRING(years(j)),1)+' is: '+STRTRIM(STRING(SKEWNESS(pdf)),1)
