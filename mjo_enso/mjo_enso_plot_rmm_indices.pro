...
years=[2009,2009]   
start_days=[283,344] ; Julian dates
stop_days=[329,390]  ; Julian dates
...
FOR i=0,n_periods-1 DO BEGIN
   ...
   obs_rmm1(i,0:counts(i)-1)=REFORM(OPEN_AND_EXTRACT(rmm_infile,'rmm1_ts',$
                                                     offset=[this_start],count=[counts(i)]))
   obs_rmm2(i,0:counts(i)-1)=REFORM(OPEN_AND_EXTRACT(rmm_infile,'rmm2_ts',$
                                                     offset=[this_start],count=[counts(i)]))
...
psfile='/home/ss901165/idl/mjo_enso/mjo_enso_plot_rmm_indices.ps'
...
GLEGEND,labels=REVERSE(['2009/10/10 to 2009/11/25','2009/12/10 to 2010/01/25']),$
        COL=REVERSE(FSC_COLOR(colors(0:n_periods-1))),LEGXOFFSET=1700,LEGYOFFSET=18000
GLEGEND,labels=REVERSE(['Start','Every five days','Finish']),SYM=REVERSE([3,5,1]),LEGXOFFSET=-9000,LEGYOFFSET=18300,$
        LENGTH=0
PSCLOSE
