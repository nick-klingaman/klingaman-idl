...
; cutoff=  The number of Number of EOFs to retain (max = no. time
; points)
...
for i=0L,nlons*nlats-1 do begin
...
; but that Lambda is the Eigenvalue matrix of X.X^T (W=sqrt(Lambda) in
; svd)

tr=0
for jk=0L,nlons*nlats-1 do begin
...
pro svdsrt,w,u,v,row=row,column=column
                                ; sorts singular values and left and
                                ; right singular vectors after call to
...
if (keyword_set(column)) then begin
 for k=0L, su(1)-1 do u(k,*)=u(k,index)
 for k=0L, sv(1)-1 do v(k,*)=v(k,index)
endif else begin
 for k=0L, su(2)-1 do u(*,k)=u(index,k)
 for k=0L, sv(2)-1 do v(*,k)=v(index,k)
endelse
return
end
