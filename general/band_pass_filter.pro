FUNCTION band_pass_filter,x,shortest_period,longest_period

; This function accepts a one-dimensional array and performs a
; band-pass filter on it to eliminate all periods shorter (all
; frequencies higher) than the specified "shortest period" and all
; periods longer than the specified "longest period."  It
...
;   band-pass filter.  For example, if x has a sampling frequency of 3
;   days and you want to retain periods longer than 30 days, then
;   shortest_period should be 10, not 30.
;
;   longest_period :: the longest period (in units of the interval
;   between the data points in x) that will be retained by the
;   band-pass filter.
...
f_low = 1./FLOAT(longest_period)                           ; Frequencies higher than f_low will be passed.   
...
x = [REVERSE(x(nx-n_terms*2:nx-1)),x,REVERSE(x(0:n_terms*2-1))]
; PRINT,N_ELEMENTS(x)
