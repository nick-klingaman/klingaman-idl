...
NCDF_VARGET,id,time_id,alltime

n_time = N_ELEMENTS(time)
n_lat = N_ELEMENTS(lat)
print,'n_lat from read_cmap_precip', n_lat
STOP
...
; function.
start_julian = [0,date(0),date(1)]
stop_julian = [0,date(2),date(3)]
...
start_hours = JULIAN_TO_HOURS(start_hours,start_cmap)
stop_hours = JULIAN_TO_HOURS(stop_hours,start_cmap)

; Figure out the starting and ending time points for reading the CMAP data.
start_time = where(time eq (MIN(time-start_hours)))
stop_time = where(time eq (MIN(time-stop_hours)))

time = alltime(start_time:stop_time)
help,time

; Read the CMAP data.
NCDF_VARGET,id,precip_id,precip,OFFSET=[0,0,start_time-1],COUNT=[n_lon,n_lat,stop-start_time+1]
