PRO lanczos_filter_file_timeyear,infile,varname,upper_limit,lower_limit,outfile,remove_mean=remove_mean,continuous=continuous,field_multiplier=field_multiplier
...
FOR i=0,var_struct.ndims-1 DO BEGIN
    ...
    IF dim_name EQ 'longitude' OR dim_name EQ 'lon' OR dim_name EQ 'x' OR dim_name EQ 'longitude_1' THEN BEGIN
       lon_dim=var_struct.dim(i)
       lon_name=dim_name
       nlon=dim_size
    ENDIF ELSE IF dim_name EQ 'latitude' OR dim_name EQ 'lat' OR dim_name EQ 'y' OR dim_name EQ 'latitude_1' THEN BEGIN
       lat_dim=var_struct.dim(i)
       lat_name=dim_name
       nlat=dim_size
    ENDIF ELSE IF dim_name EQ 'time' OR dim_name EQ 't' OR dim_name EQ 't_1' THEN BEGIN
...
IF (nmem GT 0) THEN BEGIN
   varids(4)=NCDF_VARDEF(outid,varname,[dimids(3),dimids(2),dimids(1),dimids(0)])
ENDIF ELSE $
   varids(4)=NCDF_VARDEF(outid,varname,[dimids(0),dimids(1),dimids(3)])
...
;               STOP
            ...
            FOR m=0,nmem-1 DO BEGIN
               ...
               NCDF_VARPUT,outid,varids(4),put_to_netcdf,offset=[m,0,j,i],$
                           count=[1,ntime,1,1]
            ENDFOR
            ;print,'--------> ... complete.'
         ENDFOR
