PRO plot_tropical_cyclone_densities_7901higam_tden_regression
...
longitude_vort=OPEN_AND_EXTRACT(input_file,'long')
latitude_vort=OPEN_AND_EXTRACT(input_file,'lat')
...
higem_nyears=23
track_density=OPEN_AND_EXTRACT(input_file,'gden',$
                                    offset=[tcbox_tx(1),tcbox_tx(0),0],$
                                    count=[n_lon,n_lat,higem_nyears])
vort=OPEN_AND_EXTRACT(input_file,'tden',$
...
FOR i=0,n_lon_vort-1 DO BEGIN
   FOR j=0,n_lat_vort-1 DO BEGIN
      regressions(i,j)=REGRESS(TC_area_average(*),REFORM(vor(i,j,*)),CORRELATION=temp)
...
contour_levels=[-8,-6,-4,-2,0,2,4,6,8]

; Define the path to the PostScript file you want to create
psfile='/home/swu07adk/higam/7901higam_tden_gden_regression.ps'
...
; to normal text.
CON,X=longitude_vort,Y=latitude_vort,FIELD=regressions,/NOLINES,$
    TITLE='Regression of track density across whole region on genesis density in area [-15,150,-5,180] using HiGAM'
...
GPLOT, X=-120, Y=8
xpts=[150, 150, 180, 180, 150]
ypts=[-5, -15, -15, -5, -5]
GPLOT, X=xpts, Y=ypts, THICK=150
