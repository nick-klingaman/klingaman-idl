PRO spcam_ocnTcorr_make_smooth_correction,infile,outfile,in_varname,in_zname,out_varname,n_days_per_year,mean_length
...
;            sfcorr_in=OPEN_AND_EXTRACT(infile,in_varname,offset=[i,j,0,y*n_days_per_year],count=[1,1,n_z,n_days_per_year])
         IF sfcorr_in(0,j,0,0) lt 1e19 THEN BEGIN
            FOR k=0,n_z-1 DO BEGIN
               ...
               IF TOTAL(where(ABS(temp) ge 100)) ge 0 THEN BEGIN
                  print,'Large positive values detected at ',i,j,k
                  bad=where(ABS(temp) ge 100)
